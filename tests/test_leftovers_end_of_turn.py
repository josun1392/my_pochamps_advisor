...
def _projection(pre, sides=("self", "opponent")):
    ...
    return {"schema_version": "detached-leftovers-event-target-order-v1", "status": "known", "session_id": "leftovers-eot", "event_family": "ResidualItemTier5", "source_branch_fingerprint": fingerprint_transition_preview_state(state), "ordered_active_owners": [_owner_id(state, side) for side in sides], "provenance": "trusted_canonical_showdown_leftovers_residual_target_order"}
...
def test_cross_owner_leftovers_uses_frozen_order_and_invalid_authority_fails_closed():
    ...
    assert _project_bounded_eot(pre_end_of_turn=pre, weather_event_target_order={"schema_version": "detached-weather-event-target-order-v1", "status": "known", "session_id": "leftovers-eot", "event_family": "Weather", "source_branch_fingerprint": fingerprint_transition_preview_state(pre["next_state"]), "ordered_active_owners": [_owner_id(pre["next_state"], side) for side in ("self", "opponent")], "provenance": "trusted_canonical_showdown_weather_event_target_order"}, leftovers_event_target_order=None) == {"status": "incomplete", "reason": "cross_owner_leftovers_order_unrepresented"}
    stale = deepcopy(order); stale["source_branch_fingerprint"] = "stale"
    assert project_cross_owner_weather_end_of_turn(pre_end_of_turn=pre, weather_event_target_order={"schema_version": "detached-weather-event-target-order-v1", "status": "known", "session_id": "leftovers-eot", "event_family": "Weather", "source_branch_fingerprint": fingerprint_transition_preview_state(pre["next_state"]), "ordered_active_owners": [_owner_id(pre["next_state"], side) for side in ("self", "opponent")], "provenance": "trusted_canonical_showdown_weather_event_target_order"}, leftovers_event_target_order=stale) == {"status": "rejected", "reason": "stale_or_foreign_leftovers_event_target_order"}
