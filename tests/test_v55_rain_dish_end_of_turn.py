"""Bounded Rain Dish recovery at the explicit first end-of-turn phase."""
...
def _known_rain_authority(manager, boundary, *, opponent_ability="pressure", weather="rain"):
    _apply(manager, _ability(boundary, "self", "pikachu", "rain-dish"))
    _apply(manager, _ability(boundary, "opponent", "eevee", opponent_ability))
    _apply(manager, _weather(boundary, weather))


def _result(manager):
    return manager.read_state()["state"]["rain_dish_end_of_turn_context"][-1]


def test_rain_dish_recovers_authoritative_hp_and_clamps_at_maximum():
    manager, boundary = _manager(hp=95), _boundary()
    _known_rain_authority(manager, boundary)
...
def test_rain_dish_full_hp_non_rain_and_non_rain_dish_are_resolved_without_recovery():
    full, boundary = _manager(hp=100), _boundary()
    _known_rain_authority(full, boundary)
    _apply(full, _phase(boundary))
    assert _result(full)["outcome"] == "already_full_hp" and _result(full)["recovery"] == 0

    dry, boundary = _manager(), _boundary()
    _known_rain_authority(dry, boundary, weather="sun")
    _apply(dry, _phase(boundary))
    assert dry.read_state()["state"].get("rain_dish_end_of_turn_context", []) == []
    ...
    _apply(different_ability, _weather(boundary, "rain"))
    _apply(different_ability, _phase(boundary))
    assert different_ability.read_state()["state"].get("rain_dish_end_of_turn_context", []) == []


def test_rain_dish_weather_suppression_and_unknown_authority_remain_conservative():
    for suppressor, outcome in (("cloud-nine", "suppressed_by_weather_ability"), ("air-lock", "suppressed_by_weather_ability"), ("neutralizing-gas", "suppressed_by_neutralizing_gas")):
        ...
        _known_rain_authority(manager, boundary, opponent_ability=suppressor)
        _apply(manager, _phase(boundary))
    ...
    _apply(weather_unknown, _ability(boundary, "self", "pikachu", "rain-dish"))
    ...
    _apply(suppression_unknown, _ability(boundary, "self", "pikachu", "rain-dish"))
    _apply(suppression_unknown, _weather(boundary, "rain"))
    _apply(suppression_unknown, _phase(boundary))
    assert _result(suppression_unknown)["reason"] == "current_ability_unknown"


def test_rain_dish_faint_and_unordered_same_owner_residual_do_not_fabricate_recovery():
    fainted, boundary = _manager(hp=0, fainted=True), _boundary()
    _apply(fainted, _ability(boundary, "opponent", "eevee", "pressure"))
    _apply(fainted, _weather(boundary, "rain"))
    _apply(fainted, _phase(boundary))
    assert fainted.read_state()["state"].get("rain_dish_end_of_turn_context", []) == []

    ordered, boundary = _manager(condition="poison"), _boundary()
    _known_rain_authority(ordered, boundary)
