...
@pytest.mark.parametrize("move", ["psywave", "counter", "mirror-coat", "metal-burst", "bide", "comeuppance", "fissure", "guillotine", "horn-drill", "sheer-cold"])
...
@pytest.mark.parametrize("move", ["endeavor", "final-gambit"])
def test_hp_special_moves_do_not_duplicate_fixed_damage_assessments(move):
    assert build_fixed_damage_assessment({"move_id": move}, None) is None
