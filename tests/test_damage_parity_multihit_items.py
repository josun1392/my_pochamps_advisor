...
def test_loaded_dice_bullet_seed_min_4_hits() -> None:
    ...
    _assert_multihit_parity(
        _request("cinccino", "pikachu", "bullet-seed", hits=hits, attacker_item="loaded-dice")
    )


def test_loaded_dice_rock_blast_max_5_hits() -> None:
    move = MultiHitMove("rock-blast")
    attacker = MultiHitAttacker(item="loaded-dice")
    hits = resolve_hit_count(move, attacker, mode="max")

    assert hits == 5
    _assert_multihit_parity(
        _request("cinccino", "charizard", "rock-blast", hits=hits, attacker_item="loaded-dice")
    )


def test_loaded_dice_icicle_spear_min_4_hits() -> None:
    move = MultiHitMove("icicle-spear")
    attacker = MultiHitAttacker(item="loaded-dice")
    hits = resolve_hit_count(move, attacker, mode="min")

    assert hits == 4
    _assert_multihit_parity(
        _request("mamoswine", "charizard", "icicle-spear", hits=hits, attacker_item="loaded-dice")
    )


def test_skill_link_beats_loaded_dice() -> None:
    move = MultiHitMove("bullet-seed")
