...
from advisor.damage.multihit import MultiHitMove, calc_multihit_damage_rolls
...
MOVES = {
    "triple-kick": ("fighting", "physical", 10, "normal", 3),
    "triple-axel": ("ice", "physical", 20, "normal", 3),
}
...
def _request(
    ...
    *,
    ...
    return {
        ...
        "move": {"name": move, "is_critical": False, "is_z": False, "is_max": False, "hits": 3},
        "field": {
            "weather": None,
            "terrain": None,
            "is_gravity": False,
...
def _assert_multihit_parity(request_data: dict) -> None:
    ...
    move = MultiHitMove(
        request.move.name,
        multihit=3,
        base_power=ctx.move_power,
        bp_escalation=True,
    )

    py_rolls = calc_multihit_damage_rolls(ctx, hit_count=3, move=move)

    assert py_rolls == js_response.damage_rolls


def test_triple_kick_3_hits_escalating_bp() -> None:
    _assert_multihit_parity(_request("hitmonlee", "pikachu", "triple-kick"))


def test_triple_axel_3_hits_escalating_bp() -> None:
    _assert_multihit_parity(_request("hitmonlee", "pikachu", "triple-axel"))


def test_triple_kick_with_technician_all_hits_boosted() -> None:
    _assert_multihit_parity(
        _request("hitmonlee", "pikachu", "triple-kick", attacker_ability="technician")
    )


def test_triple_axel_with_technician_all_hits_boosted() -> None:
    _assert_multihit_parity(
        _request("hitmonlee", "pikachu", "triple-axel", attacker_ability="technician")
    )
