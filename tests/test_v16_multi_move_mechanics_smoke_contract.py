...
def test_modifier_fixture_pair_preserves_candidate_local_known_and_fail_closed_evidence():
    ...
    assert [row["mechanics_result"].get("applied_damage_modifiers") for row in combined] == [
        ...
        ["rain_fire_reduction", "light_screen_reduction"],
        None,
    ]
    mixed = _prepared(MODIFIER_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert mixed[0]["mechanics_result"]["status"] == "insufficient_context"
    assert mixed[1]["mechanics_result"]["unsupported_reason"] == "battle_format"
    assert mixed[2]["mechanics_result"]["damage_model"] == "level_based_fixed"
    assert "applied_damage_modifiers" not in mixed[2]["mechanics_result"]
...
def test_ability_fixture_pair_preserves_static_matching_and_level_fixed_control():
    ...
    assert [row["mechanics_comparison"]["rank"] for row in supported] == [1, 2, 3]
    unsupported = _prepared(ABILITY_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert unsupported[0]["mechanics_result"]["unsupported_reason"] == "ability_modifier"
    assert unsupported[1]["mechanics_result"]["damage_model"] == "level_based_fixed"
    assert unsupported[1]["mechanics_comparison"]["rank"] == 1


def test_offline_ability_authority_variants_never_call_provider_or_default_to_absence():
    variants = offline_ability_authority_variants()
...
def test_defender_ability_fixture_pair_preserves_matching_fixed_hit_and_level_fixed_control():
    ...
    assert [row["mechanics_comparison"]["rank"] for row in supported] == [1, 2, 3]
    unsupported = _prepared(DEFENDER_ABILITY_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert unsupported[0]["mechanics_result"]["unsupported_reason"] == "defender_ability_modifier"
    assert unsupported[1]["mechanics_result"]["damage_model"] == "level_based_fixed"
    assert unsupported[1]["mechanics_comparison"]["rank"] == 1
...
def test_stage_fixture_pair_preserves_physical_special_mapping_and_level_fixed_control():
    ...
    assert supported[1]["mechanics_result"]["stat_stage_evidence"]["offensive_stage_stat"] == "special-attack"
    assert "stat_stage_evidence" not in supported[2]["mechanics_result"]
    incomplete = _prepared(STAGE_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["mechanics_result"]["status"] == "insufficient_context"
    assert incomplete[1]["mechanics_comparison"]["rank"] == 1
...
def test_grounded_terrain_fixture_pair_keeps_candidate_local_q12_evidence():
    ...
    assert supported[2]["mechanics_result"]["hit_count"] == 2
    assert supported[3]["mechanics_result"]["damage_model"] == "level_based_fixed"
    incomplete = _prepared(TERRAIN_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["mechanics_result"]["missing_inputs"] == ["self.grounded"]
    assert incomplete[1]["mechanics_comparison"]["rank"] == 1
...
def test_trick_room_fixture_pair_keeps_priority_and_reverse_speed_evidence_isolated():
    ...
    assert supported[0]["action_order"]["status"] == "acts_first"
    assert supported[1]["action_order"]["reason"] == "priority_advantage"
    assert supported[2]["mechanics_result"]["damage_model"] == "level_based_fixed"
    incomplete = _prepared(TRICK_ROOM_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["action_order"]["missing_inputs"] == ["trick_room"]
...
def test_tailwind_fixture_pair_keeps_side_owned_speed_evidence_isolated():
    ...
    assert supported[0]["action_order"]["tailwind_adjustment_applied"] is True
    assert supported[0]["action_order"]["trick_room"] == "active"
    assert supported[1]["action_order"]["reason"] == "priority_advantage"
    assert supported[2]["mechanics_result"]["damage_model"] == "level_based_fixed"
    incomplete = _prepared(TAILWIND_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["action_order"]["missing_inputs"] == ["self_tailwind"]
    assert incomplete[1]["action_order"]["reason"] == "priority_advantage"
    assert incomplete[1]["mechanics_comparison"]["rank"] == 1
...
def test_paralysis_fixture_pair_keeps_side_owned_speed_evidence_isolated():
    ...
    assert supported[0]["action_order"]["opponent_paralysis"] == "not_paralyzed"
    assert supported[0]["action_order"]["paralysis_speed_adjustment_applied"] is True
    assert supported[0]["action_order"]["status"] == "acts_second"
    assert supported[1]["action_order"]["reason"] == "priority_advantage"
    assert "self_paralysis" not in supported[1]["action_order"]
    assert supported[2]["mechanics_result"]["damage_model"] == "level_based_fixed"
    incomplete = _prepared(PARALYSIS_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["action_order"]["missing_inputs"] == ["self_paralysis"]
    assert incomplete[1]["action_order"]["reason"] == "priority_advantage"
    assert incomplete[1]["mechanics_comparison"]["rank"] == 1
...
def test_prankster_fixture_pair_keeps_status_priority_and_unknown_authority_isolated():
    ...
    assert supported[0]["action_order"]["self_priority"] == 1
    assert "self_prankster_applied" not in supported[1]["action_order"]
    assert supported[2]["action_order"]["reason"] == "priority_advantage"
    incomplete = _prepared(PRANKSTER_FIXTURES[1])["recommendation_request"]["candidate_comparisons"]
    assert incomplete[0]["action_order"]["missing_inputs"] == ["self_priority_ability"]
    assert incomplete[1]["action_order"]["reason"] == "priority_advantage"
    assert incomplete[1]["mechanics_comparison"]["rank"] == 1
...
def test_gale_wings_fixture_pair_keeps_exact_hp_authority_and_priority_isolated():
    ...
    assert supported[0]["action_order"]["self_base_priority"] == 0
    assert supported[0]["action_order"]["self_priority"] == 1
    assert "self_gale_wings_applied" not in supported[1]["action_order"]
    assert supported[3]["mechanics_result"]["damage_model"] == "level_based_fixed"
...
def test_offline_grounded_terrain_variants_remain_fail_closed_without_provider_calls():
    ...
    assert variants == {
        "provider_calls": 0,
        "variants": {
            "grounded": {"mechanics_status": "known", "applied": ["terrain_electric_boost"], "rank": 1},
            "ungrounded": {"mechanics_status": "known", "applied": [], "rank": 1},
            "unknown": {"mechanics_status": "insufficient_context", "applied": None, "rank": None},
            "malformed": {"mechanics_status": "unsupported_mechanic", "applied": None, "rank": None},
        },
        "no_usable_cycle_status": "no_selectable_candidates",
    }
...
def test_offline_defender_ability_authority_variants_never_call_provider_or_default_to_no_effect():
    ...
    assert variants["stale_context_status"] == "known"
    assert variants["candidate_mismatch_errors"] == ["multi_provider_binding_invalid"]
    assert variants["no_usable_cycle_status"] == "ready"
...
def test_psychic_terrain_priority_block_fixture_pair_excludes_blocked_candidates_and_binds_only_control_evidence(capsys):
    ...
    assert supported["candidate_comparisons"][0]["move_success"]["block_reason"] == "psychic_terrain_priority"
    incomplete = _prepared(PSYCHIC_TERRAIN_PRIORITY_BLOCK_FIXTURES[1])["recommendation_request"]
    assert incomplete["candidate_comparisons"][0]["move_success"]["missing_inputs"] == ["opponent.grounded"]
