...
```text
T3, continue after v7.1 Deterministic Turn Order Context Design.
...
- Recommended next milestone:
  ...
  - Alternative: v7.2 Deterministic Turn Order Context Helper
- Reason:
  ...
  - Next should lock the turn order context payload contract first if safety is preferred, or implement a narrow helper if implementation momentum is preferred.
  ...
  - Do not implement resolved turn order, speed tie resolver, RNG resolver, item consumption, HP update, exact trigger resolution, or opponent set inference in v7.2.
