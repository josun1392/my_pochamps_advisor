...
```text
T3, continue after v6.18 UI Dev Flag Implementation.
...
- Recommended next milestone:
  ...
  - Alternative: v6.19 UI Copy Polish
- Reason:
  ...
  - Next should manually QA the dev flag on/off UI behavior. Any actual Gemini call from UI still requires explicit T1 approval and should be at most one controlled smoke.
...
- Larger next direction: v6.19 UI Dev Flag Smoke / Manual QA, with v6.19 UI Copy Polish as the no-Gemini alternative. Controlled UI Gemini Smoke requires explicit T1 approval and should remain at most one call with no retry. Keep additional actual Gemini calls disabled unless T1/T2 explicitly approve a separate one-call smoke.
