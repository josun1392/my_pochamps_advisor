...
```text
T3, continue after v7.9 UI / Flag Integration Design.
...
- Recommended next milestone:
  ...
  - Alternative: v7.10 UI Copy Polish
- Reason:
  ...
  - Next should implement the default-off UI flag mapping so checked means `enable_turn_pipeline=True` and `enable_turn_order_context=True`, while unchecked means both remain false.
  ...
  - Do not implement resolved turn order, speed tie resolver, RNG resolver, item consumption, HP update, exact trigger resolution, or opponent set inference in v7.10.
