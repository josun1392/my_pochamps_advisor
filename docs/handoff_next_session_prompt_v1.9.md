...
```text
T3, continue after v8.1 Opponent Move Context Payload Contract.
...
- Reason:
  ...
  - Next should implement a minimal helper that produces the v8.1 shape from explicit known/candidate move data only.
  ...
  - Do not implement resolved turn order, speed tie resolver, RNG resolver, item consumption, HP update, exact trigger resolution, opponent set inference, hidden moveset inference, selected opponent move inference, EV/IV/nature inference, hidden item inference, or weather/terrain/boost inference in v8.2.
