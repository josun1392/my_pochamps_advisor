...
```text
T3, continue after v7.12 Controlled UI Gemini Smoke Design.
...
- Recommended next milestone:
  ...
  - Alternative: v7.13 Turn Order UI Integration Closure
- Reason:
  ...
  - Next may run the controlled UI Gemini smoke only if T1 explicitly approves the one-call smoke.
  - Do not run another actual Gemini call unless T1 explicitly approves v7.13.
  - No Vertex AI call.
  ...
  - Do not implement resolved turn order, speed tie resolver, RNG resolver, item consumption, HP update, exact trigger resolution, or opponent set inference in v7.13.
