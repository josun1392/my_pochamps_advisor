...
`current_type_context` is a request-start, side-owned authority for a Pokémon's **current** typing. It is deliberately separate from repository species/base typing: species metadata is never a fallback when explicit current-type authority is unknown or malformed; formula Q12 retains a documented legacy compatibility path only when this context is omitted.
