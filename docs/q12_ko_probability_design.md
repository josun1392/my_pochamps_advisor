...
`supported-exact-q12-ko-probability` verifies a selected Formula Q12 candidate with trusted exact HP and non-trivial server-owned cumulative evidence. `unknown-ko-probability-hp-with-formula-control` verifies that explicit unknown HP leaves probability insufficient while retaining the formula candidate's existing usability. Both fixtures preflight all authority locally and permit the provider only to return the minimal candidate selection; prompts, payloads, raw rolls, and provider responses are not retained in this document.
