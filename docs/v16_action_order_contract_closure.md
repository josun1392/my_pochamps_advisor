...
Sanitized actual-grounding fixtures cover Prankster, Gale Wings, Triage, and the Psychic Terrain priority-block gate. Offline coverage additionally fixes the three defender-ability sources, target-side ownership, source short-circuiting, and their non-selectable candidate behavior. The Psychic pair keeps a blocked positive-priority opposing action server-owned and non-selectable, while the provider may select only from the remaining rankable controls; the unknown-grounded fixture keeps the priority candidate unavailable without converting the unknown state to allowed or blocked. Their offline/fake-provider contracts remain regression coverage for the minimal provider boundary. Further work should be a separate bounded proposal for one unsupported priority-blocking or move-success mechanic, rather than expanding this contract implicitly.
