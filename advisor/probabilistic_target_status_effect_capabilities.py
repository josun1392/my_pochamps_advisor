...
def _types_entry(value: Mapping[str, Any], rule: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(value["values"]) & set(rule["target_type_policy"]["ineligible"]):
        return _row("target_types", "ineligible", source_value=",".join(value["values"]), reason=f"target_type_{rule['effect']['condition']}_immunity")
...
def _valid_rule(value: Any) -> bool:
    ...
    condition = effect.get("condition") if isinstance(effect, Mapping) else None
    expected_immunity = {"paralysis": ["electric"], "burn": ["fire"]}.get(condition)
    return isinstance(value.get("rule_id"), str) and bool(value["rule_id"]) and isinstance(value.get("move_id"), str) and bool(value["move_id"]) and value.get("category") in {"physical", "special"} and isinstance(value.get("target_scope"), str) and bool(value["target_scope"]) and isinstance(value.get("effect_chance"), int) and not isinstance(value["effect_chance"], bool) and 1 <= value["effect_chance"] <= 99 and isinstance(effect, Mapping) and effect == {"owner": "target", "condition": condition} and condition in {"paralysis", "burn"} and isinstance(conditions, Mapping) and conditions == {"requires_successful_damaging_hit": True, "blocked_by_substitute": True, "target_must_survive": True} and value.get("required_runtime_slots") == ["target.current_condition", "target.current_types", "attacker_ability", "target_ability", "target_item", "field.terrain", "target.groundedness"] and value.get("suppressor_slots") == ["attacker_ability", "target_ability", "target_item"] and ability_policy(policies[0]) and ability_policy(policies[1]) and isinstance(policies[2], Mapping) and isinstance(policies[2].get("suppresses"), list) and all(isinstance(item, str) and item for item in policies[2]["suppresses"]) and isinstance(policies[3], Mapping) and policies[3].get("ineligible") == expected_immunity
