...
def canonical_contact_reactive_protection_metadata(move_id: Any)->dict[str,Any]|None:
 if move_id not in {"silk-trap","kings-shield","obstruct"}: return None
 try: data=json.loads(_PATH.read_text(encoding="utf-8")); row=data.get("moves",{}).get(move_id)
 except (OSError,json.JSONDecodeError): return None
 expected={"silk-trap":{"owner":"blocked_attacker","stat":"speed","delta":-1},"kings-shield":{"owner":"blocked_attacker","stat":"attack","delta":-1},"obstruct":{"owner":"blocked_attacker","stat":"defense","delta":-2}}[move_id]
...
def canonical_obstruct_metadata(move_id: Any)->dict[str,Any]|None:
 return canonical_contact_reactive_protection_metadata(move_id) if move_id=="obstruct" else None
