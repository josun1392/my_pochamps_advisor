...
def canonical_silk_trap_metadata(move_id: Any)->dict[str,Any]|None:
 if move_id!="silk-trap": return None
 try: data=json.loads(_PATH.read_text(encoding="utf-8")); row=data.get("moves",{}).get(move_id)
 except (OSError,json.JSONDecodeError): return None
 if not isinstance(row,Mapping) or row.get("protects_self") is not True or row.get("blocks_supported_direct_damage") is not True or row.get("protection_kind")!="ordinary_self_protection" or row.get("reactive_contact_effect")!={"owner":"blocked_attacker","stat":"speed","delta":-1}: return None
