"""Pinned canonical Lum Berry family for Fling major-status + confusion cure."""
...
SCHEMA_VERSION = "canonical-fling-lum-major-status-confusion-cure-berry-v1"
...
_ITEM_ID = "lum-berry"
_SHOWDOWN_ID = "lumberry"
_REMOVABLE_CONDITIONS = ("burn", "poison", "toxic", "paralysis", "sleep", "freeze")


@lru_cache(maxsize=1)
def _source_binding() -> tuple[dict[str, Any] | None, str | None]:
    if COMMIT != FLING_BERRY_COMMIT:
        return None, "fling_lum_commit_binding_mismatch"
    try:
        raw = _VENDOR.read_bytes()
    except OSError:
        return None, "fling_lum_pinned_source_missing"
    digest = hashlib.sha256(raw).hexdigest()
    if digest != EXPECTED_ITEMS_SHA256:
        return None, "fling_lum_pinned_source_hash_mismatch"
    text = raw.decode("utf-8")
    marker = f"\n\t{_SHOWDOWN_ID}: {{"
    start = text.find(marker)
    if start < 0:
        return None, "fling_lum_source_item_missing"
    ...
    if any(token not in block for token in required):
        return None, "fling_lum_status_confusion_cure_contract_changed"
    return {
        ...
        "held_triggers": "status_or_confusion_then_eat_item",
        "intrinsic_on_eat": "cure_status_and_remove_confusion_volatile",
    }, None


def resolve_canonical_fling_lum_major_status_confusion_cure_berry(
    item_id: Any,
) -> dict[str, Any]:
    ...
    if not isinstance(item_id, str) or not item_id:
        return {"status": "incomplete", "reason": "fling_lum_item_identity_unknown"}
    if item_id != _ITEM_ID:
        return {
            "status": "not_applicable",
            "item_id": item_id,
            "reason": "fling_item_not_lum_major_status_confusion_cure_berry",
        }
    ...
    if berry.get("status") != "resolved":
        return {
            "status": "rejected",
            "item_id": item_id,
            "reason": berry.get("reason", "fling_lum_berry_identity_binding_invalid"),
        }
    if (
        ...
        return {
            "status": "rejected",
            "item_id": item_id,
            "reason": "fling_lum_manifest_binding_invalid",
        }
    return {
        "status": "resolved",
        ...
        "effect_family": "lum_major_status_confusion_cure",
        "fling_base_power": 10,
        "is_berry": True,
        "removable_conditions": _REMOVABLE_CONDITIONS,
        "intrinsic_on_eat": "cure_major_status_and_remove_confusion_volatile",
        "berry_identity_authority": berry,
        "fling_item_metadata": metadata,
        "source_provenance": source,
        "provenance": "pinned_showdown_fling_lum_major_status_confusion_cure_berry_v1",
    }
