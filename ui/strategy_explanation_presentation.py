...
def _probabilistic_target_stage_effect_summaries(value: Any, candidate_id: str) -> bool:
    if not isinstance(value, (tuple, list)):
        return False
    for row in value:
        ...
        if not isinstance(uncertainty, Mapping) or uncertainty.get("status") != "resolved" or uncertainty.get("schema_version") != "deterministic-predictive-probabilistic-target-stage-effect-uncertainty-v1" or uncertainty.get("move_id") != candidate_id.removeprefix("attack:") or not isinstance(probability, Mapping) or not isinstance(leaves, (tuple, list)) or len(leaves) != 16:
            return False
        numerator, denominator = probability.get("numerator"), probability.get("denominator")
        if not isinstance(numerator, int) or isinstance(numerator, bool) or not isinstance(denominator, int) or isinstance(denominator, bool) or denominator <= 0 or not 0 <= numerator <= denominator:
            return False
...
def _probabilistic_target_stage_effect_labels(values: Any) -> list[str]:
    if not isinstance(values, (tuple, list)):
        return []
    labels = []
    for row in values:
        ...
        if eligible and numerator > 0:
            labels.append(f"{path} 생존 피해 roll {len(eligible)}/16개에서 상대 SpD -1 가능: {numerator}/{denominator} (미발동 {denominator - numerator}/{denominator})")
