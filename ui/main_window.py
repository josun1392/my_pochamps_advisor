...
class MainWindow(QMainWindow):
    ...
    def _open_confusion_action_result_confirmation(self) -> None:
        ...
        if result.get("status") == "resolved" and isinstance(retained, dict):
            ...
            self_hit = result.get("retained_self_hit_prediction")
            self_hit_storage = dict(getattr(self, "_historical_confusion_self_hit_predictions", {}))
            if isinstance(self_hit, dict) and self_hit.get("status") == "resolved":
                self_hit_storage[key] = deepcopy(self_hit)
            else:
                self_hit_storage.pop(key, None)
            self._historical_confusion_self_hit_predictions = self_hit_storage
    ...
    @Slot()
    def _open_confusion_self_hit_damage_confirmation(self) -> None:
        manager = getattr(self, "_observation_runtime_session_manager", None)
        session_id = MainWindow._active_session_id(self)
        turn_number = getattr(self, "_current_trusted_turn_number", None)
        if (
            ...
            self.statusBar().showMessage("Confusion self-hit damage confirmation failed: active turn/session unavailable")
            return
        storage = getattr(self, "_historical_confusion_self_hit_predictions", {})
        candidates = [
            deepcopy(value)
            for key, value in storage.items()
            if (
                isinstance(key, tuple) and len(key) == 4
                and key[0] == session_id and key[1] == turn_number
                and isinstance(value, dict) and value.get("status") == "resolved"
            )
        ] if isinstance(storage, dict) else []
        if len(candidates) != 1:
            self.statusBar().showMessage(
                "Confusion self-hit damage confirmation failed: exact historical self-hit prediction unavailable"
                if not candidates
                else "Confusion self-hit damage confirmation failed: ambiguous self-hit opportunity"
            )
            return
        retained = candidates[0]
        rolls = retained.get("canonical_damage_rolls")
        if not isinstance(rolls, (tuple, list)) or len(rolls) != 16:
            self.statusBar().showMessage("Confusion self-hit damage confirmation failed: exact roll family unavailable")
            return
        hp_before = rolls[0].get("hp_before")
        if not isinstance(hp_before, int) or isinstance(hp_before, bool) or hp_before < 0:
            self.statusBar().showMessage("Confusion self-hit damage confirmation failed: exact pre-hit HP unavailable")
            return
        hp_after, ok = QInputDialog.getInt(
            self, "Confirm Confusion Self-Hit Damage",
            f"Observed HP after confusion self-hit (before: {hp_before})",
        ...
        disguise_labels = {
            ...
            "Already broken before self-hit": "already_broken",
            "Intact before self-hit → broke from self-hit": "intact_to_broken",
        }
        label, ok = QInputDialog.getItem(
            self, "Confirm Confusion Self-Hit Damage",
            "Observed Disguise result",
            list(disguise_labels), 0, False,
        )
        if not ok:
            return
        disguise_outcome = disguise_labels[label]
        actor = retained.get("actor") if isinstance(retained.get("actor"), dict) else {}
        if QMessageBox.question(
            self,
            "Confirm Confusion Self-Hit Damage",
            f"Confirm HP {hp_before} → {hp_after} for {actor.get('side', 'unknown')} active Pokémon?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        ) != QMessageBox.StandardButton.Yes:
            return
    ...
    def _open_current_opponent_switch_response_set_confirmation(self) -> None:
        """Collect explicit current switch facts from reducer-known target identities only."""
        manager = getattr(self, "_observation_runtime_session_manager", None)
        session_id = MainWindow._active_session_id(self)
        turn_number = getattr(self, "_current_trusted_turn_number", None)
        if not isinstance(manager, BattleObservationRuntimeSessionManager) or session_id is None or not isinstance(turn_number, int) or turn_number < 1:
            self.statusBar().showMessage("상대 교체 응답 확인 실패: 현재 세션 또는 신뢰된 턴 정보 없음")
            return
        snapshot = manager.capture_runtime_state_snapshot(session_id)
        state = snapshot.get("state") if snapshot.get("status") == "runtime_snapshot_ready" else None
        side = state.get("opponent_side") if isinstance(state, dict) else None
        ...
        active_slot = side.get("active_slot_index") if isinstance(side, dict) else None
        if not isinstance(roster, dict) or not isinstance(active_slot, int):
            self.statusBar().showMessage("상대 교체 응답 확인 실패: 현재 상대 로스터 정보 없음")
            return
        targets = [
            {"slot_index": slot, "pokemon_id": pokemon["pokemon_id"]}
            for slot, pokemon in roster.items()
            if isinstance(slot, int) and slot != active_slot and isinstance(pokemon, dict) and isinstance(pokemon.get("pokemon_id"), str) and pokemon["pokemon_id"]
        ]
        permission, accepted = QInputDialog.getItem(self, "Opponent switch permission", "Current opponent switch permission", ["permitted", "blocked", "unknown"], 0, False)
        if not accepted:
            return
        ...
        for row in targets:
            ...
            if not accepted:
                return
            confirmed_targets.append({**row, "availability": availability})
        result = admit_current_opponent_switch_response_set_observation(runtime_session_manager=manager, captured_session_id=session_id, permission=permission, targets=confirmed_targets, turn_number=turn_number)
        if result.get("status") == "resolved":
            self.statusBar().showMessage("현재 상대 교체 응답 집합 확인 완료")
        elif result.get("reason") in {"explicit_switch_permission_required", "explicit_complete_target_set_required"}:
            self.statusBar().showMessage("상대 교체 응답 집합은 명시적 완전 확인이 필요합니다")
        else:
            self.statusBar().showMessage("상대 교체 응답 확인 실패: 명시적 현재 정보가 필요합니다")
    ...
    def _capture_structured_observed_damage_confirmation(self, entry: dict) -> dict | None:
        ...
        event = {"event_kind": "direct_move_damage_observed", "session_id": captured_session_id, "attacker": owners["opponent"], "defender": owners["self"], "move_id": None, "move_slot": None, "damage_amount": damage, "hp_unit": "exact", "source": "ui_observed_damage_confirmation", "trust": "user_confirmed_observation", "observed": True, "confirmed": True, "reducer_eligibility": "evidence_only", "observation_id": observation_id, "observation_sequence": observation_sequence, "turn_number": getattr(self, "_current_trusted_turn_number", None), "turn_source": "ui_turn_number_confirmation", "turn_trust": "user_confirmed_observation", "reconciliation_eligible": False}
        linked = self._linked_executed_predictive_bundle()
        if linked.get("status") != "resolved":
            return event
        binding = linked["binding"]
