...
def _fixture(fixture_id: str) -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    if fixture_id == ACCURACY_STAGE_FIXTURES[0]:
        return [{"move_id": "high"}, {"move_id": "low"}, {"move_id": "swift"}], {"high": {"category": "physical", "power": 100, "type": "normal", "accuracy": 100, "priority": 0}, "low": {"category": "physical", "power": 40, "type": "normal", "accuracy": 70, "priority": 0}, "swift": {"category": "special", "power": 60, "type": "normal", "always_hit": True, "priority": 0}}
    if fixture_id == ACCURACY_STAGE_FIXTURES[1]:
        return [{"move_id": "tackle"}, {"move_id": "swift"}], {"tackle": {"category": "physical", "power": 40, "type": "normal", "accuracy": 90, "priority": 0}, "swift": {"category": "special", "power": 60, "type": "normal", "always_hit": True, "priority": 1}}
    ...
    if fixture_id == GALE_WINGS_FIXTURES[0]:
        return [{"move_id": "brave-bird"}, {"move_id": "slam"}, {"move_id": "quick-attack"}, {"move_id": "seismic-toss"}], {"brave-bird": {"category": "physical", "power": 120, "type": "flying", "priority": 0}, "slam": {"category": "physical", "power": 40, "type": "normal", "priority": 0}, "quick-attack": {"category": "physical", "power": 40, "type": "normal", "priority": 1}, "seismic-toss": {"category": "physical", "type": "normal", "priority": 0}, "tackle": {"category": "physical", "power": 40, "type": "normal", "priority": 0}}
    if fixture_id == GALE_WINGS_FIXTURES[1]:
        return [{"move_id": "brave-bird"}, {"move_id": "seismic-toss"}], {"brave-bird": {"category": "physical", "power": 120, "type": "flying", "priority": 0}, "seismic-toss": {"category": "physical", "type": "normal", "priority": 1}, "tackle": {"category": "physical", "power": 40, "type": "normal", "priority": 0}}
...
def _prepared(fixture_id: str) -> dict[str, Any]:
    ...
    if fixture_id == PARALYSIS_FIXTURES[1]:
        ...
        battle["stat_stage_context"] = {"current_stages": [_stage("self", "attack", 0), _stage("opponent", "defense", 0), _stage("self", "speed", 0), _stage("opponent", "speed", 0)]}
    if fixture_id == STATIC_SPEED_FIXTURES[0]:
        battle["item_profiles"] = {"my_active": {"status": "user_confirmed", "source": "user_input", "item_id": "choice-scarf"}, "opponent_active": {"status": "none", "source": "user_input"}}
        battle["stat_stage_context"] = {"current_stages": [_stage("self", "attack", 0), _stage("opponent", "defense", 0), _stage("self", "speed", 0), _stage("opponent", "speed", 0)]}
        for entry in battle["final_stat_context"]["current_final_stats"]:
            if entry.get("stat") == "speed": entry["value"] = 100 if entry.get("side") == "self" else 140
    if fixture_id == STATIC_SPEED_FIXTURES[1]:
    ...
    if fixture_id == GALE_WINGS_FIXTURES[0]:
        battle["ability_context"] = {"current_abilities": [_current_ability("gale-wings"), _current_ability("prankster", side="opponent", pokemon="eevee", slot=1)]}
        battle["current_hp_context"] = {"current_hp": [_current_hp("self", 100, 100), _current_hp("opponent", 100, 100)]}
    if fixture_id == GALE_WINGS_FIXTURES[1]:
        battle["ability_context"] = {"current_abilities": [_current_ability("gale-wings"), _current_ability("prankster", side="opponent", pokemon="eevee", slot=1)]}
        battle["current_hp_context"] = {"current_hp": [_current_hp("opponent", 100, 100)]}
    if fixture_id == TRIAGE_FIXTURES[0]:
    ...
    if fixture_id in PRIORITY_BLOCKING_ABILITY_FIXTURES:
        battle["ability_context"] = {"current_abilities": [_current_ability("armor-tail" if fixture_id == PRIORITY_BLOCKING_ABILITY_FIXTURES[0] else "unknown", side="opponent", pokemon="eevee", slot=1)]}
    ...
    if fixture_id in CURRENT_TYPE_Q12_FIXTURES:
        if fixture_id == CURRENT_TYPE_Q12_FIXTURES[0]:
            battle["current_type_context"] = {"current_types": [
                {"side": "self", "state": "known", "types": ["fire"], "status": "user_confirmed", "source": "user_confirmed_current_type", "authority_provenance": "user_confirmed_current", "confidence": "known", "provenance": _provenance("self", 0, "pikachu", source="user_confirmed_current_type")},
...
def _current_hp(side: str, current_hp: int, maximum_hp: int) -> dict[str, Any]:
    pokemon, slot = ("pikachu", 0) if side == "self" else ("eevee", 1)
    return {"side": side, "current_hp": current_hp, "maximum_hp": maximum_hp, "status": "user_confirmed", "source": "user_confirmed_current_hp", "confidence": "known", "provenance": _provenance(side, slot, pokemon, source="user_confirmed_current_hp")}


def _stage(side: str, stat: str, stage: int) -> dict[str, Any]:
    pokemon, slot = ("pikachu", 0) if side == "self" else ("eevee", 1)
    return {"side": side, "stat": stat, "stage": stage, "status": "user_confirmed", "source": "user_confirmed_current_stat_stage", "confidence": "known", "provenance": _provenance(side, slot, pokemon, source="user_confirmed_current_stat_stage")}
...
def _fixture_contract_valid(fixture_id: str, payload: Mapping[str, Any]) -> bool:
    ...
    if fixture_id == ACCURACY_STAGE_FIXTURES[0]:
        evidence = [row.get("accuracy_evidence", {}) for row in rows]
        return [item.get("adjusted_accuracy") for item in evidence[:2]] == [100, 93] and evidence[0].get("canonical_accuracy") == 100 and evidence[1].get("canonical_accuracy") == 70 and evidence[2].get("status") == "always_hits"
    if fixture_id == ACCURACY_STAGE_FIXTURES[1]:
        ordinary, always = [row.get("accuracy_evidence", {}) for row in rows]
        return ordinary.get("status") == "insufficient_context" and ordinary.get("uncertainty") == ["opponent_evasion_stage"] and always.get("status") == "always_hits" and comparisons[1].get("rank") == 1
    if fixture_id == STATUS_FIXTURES[0]:
    ...
    if fixture_id == STAGE_FIXTURES[0]:
        physical, special, fixed = [row.get("mechanics_result", {}) for row in rows]
        return [item.get("rank") for item in comparisons if isinstance(item, Mapping)] == [1, 2, 3] and physical.get("hit_count") == 2 and physical.get("stat_stage_evidence", {}).get("offensive_stage_stat") == "attack" and physical.get("stat_stage_evidence", {}).get("defensive_stage_stat") == "defense" and special.get("stat_stage_evidence", {}).get("offensive_stage_stat") == "special-attack" and special.get("stat_stage_evidence", {}).get("defensive_stage_stat") == "special-defense" and fixed.get("damage_model") == "level_based_fixed" and "stat_stage_evidence" not in fixed
    if fixture_id == STAGE_FIXTURES[1]:
    ...
    if fixture_id == TAILWIND_FIXTURES[0]:
        orders = [row.get("action_order", {}) for row in rows]
        return orders[0].get("status") == "acts_second" and orders[0].get("reason") == "speed_advantage" and orders[0].get("self_tailwind") == "active" and orders[0].get("opponent_tailwind") == "inactive" and orders[0].get("tailwind_adjustment_applied") is True and orders[0].get("trick_room") == "active" and orders[1].get("status") == "acts_first" and orders[1].get("reason") == "priority_advantage" and "tailwind_adjustment_applied" not in orders[1] and rows[2].get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and comparisons[0].get("rank") == 1
    if fixture_id == TAILWIND_FIXTURES[1]:
        orders = [row.get("action_order", {}) for row in rows]
    ...
    if fixture_id == STATIC_SPEED_FIXTURES[0]:
        orders = [row.get("action_order", {}) for row in rows]
        return orders[0].get("status") == "acts_first" and orders[0].get("self_speed_item_applied") == "choice-scarf" and orders[1].get("reason") == "priority_advantage" and "self_speed_item_applied" not in orders[1] and orders[2].get("status") == "acts_first" and orders[2].get("self_speed_item_applied") == "choice-scarf" and rows[2].get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and comparisons[2].get("rank") == 1
    if fixture_id == STATIC_SPEED_FIXTURES[1]:
        orders = [row.get("action_order", {}) for row in rows]
    ...
    if fixture_id == GALE_WINGS_FIXTURES[0]:
        orders = [row.get("action_order", {}) for row in rows]
        return orders[0].get("status") == "acts_first" and orders[0].get("self_gale_wings_applied") is True and orders[0].get("self_base_priority") == 0 and orders[0].get("self_priority") == 1 and orders[0].get("reason") == "priority_advantage" and orders[1].get("status") == "speed_tie" and "self_gale_wings_applied" not in orders[1] and orders[2].get("status") == "acts_first" and orders[2].get("reason") == "priority_advantage" and rows[3].get("mechanics_result", {}).get("damage_model") == "level_based_fixed"
    if fixture_id == GALE_WINGS_FIXTURES[1]:
        orders = [row.get("action_order", {}) for row in rows]
        return orders[0].get("status") == "insufficient_context" and orders[0].get("missing_inputs") == ["self_full_hp_authority"] and orders[1].get("status") == "acts_first" and orders[1].get("reason") == "priority_advantage" and "self_gale_wings_applied" not in orders[1] and rows[1].get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and comparisons[1].get("rank") == 1
    if fixture_id == TRIAGE_FIXTURES[0]:
        orders = [row.get("action_order", {}) for row in rows]
        return orders[0].get("status") == "acts_first" and orders[0].get("self_triage_applied") is True and orders[0].get("self_base_priority") == 0 and orders[0].get("self_priority") == 3 and orders[0].get("reason") == "priority_advantage" and orders[1].get("status") == "speed_tie" and "self_triage_applied" not in orders[1] and orders[2].get("status") == "acts_first" and orders[2].get("reason") == "priority_advantage" and rows[3].get("mechanics_result", {}).get("damage_model") == "level_based_fixed"
    if fixture_id == TRIAGE_FIXTURES[1]:
        orders = [row.get("action_order", {}) for row in rows]
    ...
    if fixture_id == PRIORITY_BLOCKING_ABILITY_FIXTURES[0]:
        blocked, control, lower = rows
        success = blocked.get("move_success", {})
        return success.get("move_success_status") == "blocked" and success.get("block_reason") == "priority_blocking_ability" and success.get("priority_block_source") == "armor_tail_priority" and success.get("armor_tail_priority_blocked") is True and success.get("psychic_terrain_priority_blocked") is not True and blocked.get("availability") == "unavailable" and blocked.get("damage", {}).get("status") == "unavailable" and "q12_damage" not in blocked and blocked.get("action_order", {}).get("status") == "acts_first" and control.get("move_success", {}).get("move_success_status") == "allowed" and control.get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and lower.get("move_success", {}).get("move_success_status") == "allowed" and lower.get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and [item.get("rank") if isinstance(item, Mapping) else None for item in comparisons] == [None, 1, 2] and payload.get("selectable_candidate_exact_set") == [{"slot_index": 1, "move": "seismic-toss"}, {"slot_index": 2, "move": "night-shade"}]
    if fixture_id == PRIORITY_BLOCKING_ABILITY_FIXTURES[1]:
        blocked, control, lower = rows
        success = blocked.get("move_success", {})
    ...
    if fixture_id == CURRENT_TYPE_Q12_FIXTURES[0]:
        ...
        evidence = formula.get("mechanics_result", {}).get("type_damage_evidence", {})
        return formula.get("move_success", {}).get("move_success_status") == "allowed" and formula.get("mechanics_result", {}).get("status") == "known" and formula.get("mechanics_result", {}).get("damage_model") == "single_hit_formula" and evidence.get("current_type_override_used") is True and evidence.get("attacker_type_authority") == "current_type_context" and evidence.get("defender_type_authority") == "current_type_context" and formula.get("availability") == "partially_evaluable" and control.get("mechanics_result", {}).get("damage_model") == "level_based_fixed" and [item.get("rank") if isinstance(item, Mapping) else None for item in comparisons] == [1, 2] and payload.get("selectable_candidate_exact_set") == [{"slot_index": 0, "move": "flamethrower"}, {"slot_index": 1, "move": "seismic-toss"}]
...
def _presentation_contract_valid(*, fixture_id: str, completed: Mapping[str, Any]) -> bool:
    ...
    if fixture_id in TAILWIND_FIXTURES:
        order = selected.get("action_order")
        if fixture_id == TAILWIND_FIXTURES[0]:
    ...
    if fixture_id in STATIC_SPEED_FIXTURES:
        order = selected.get("action_order")
        if fixture_id == STATIC_SPEED_FIXTURES[0]:
    ...
    if fixture_id in TRIAGE_FIXTURES:
        order = selected.get("action_order")
        if fixture_id == TRIAGE_FIXTURES[0]:
