...
def materialize_champions_status_gated_pair(*, strategy_d0, runtime_snapshot, base, own_action, opponent_action, own_meta, opponent_meta, orders, action_order_authority, quick_claw_action_order_authority=None, extension_authorities=None):
    ...
    def walk(plan, lineup, index, snapshot, probability, events, hp, execution=None):
        ...
        for branch in gate["branches"]:
            ...
            if target_raw.get("condition") == "freeze" and (metadata["metadata"].get("type") == "fire" or gate["move_authority"].get("self_thaw") is True):
                error = {"status": "incomplete", "reason": "target_thaw_effect_authority_required"}; return
            selected = execute_gated_selected_action(
                strategy_d0=view["strategy_d0"], runtime_snapshot=view["runtime_snapshot"], action=action,
                actor=actor, target=target, metadata_authority=metadata, extension_authorities=extension_authorities,
                pending_action=lineup[1][1], pending_metadata_authority=lineup[1][2],
                turn_local_endure_context=preceding_endure_turn_context(events) if index == 1 else None,
            )
            if selected.get("status") != "resolved": error = selected; return
            for selected_path in selected["paths"]:
                if index == 1 and events:
                    deferred = consume_deferred_protection_setup(setup_path=events[-1].get("selected_action_path"), pending_action=action, pending_metadata_authority=metadata, pending_branch_snapshot=view["runtime_snapshot"], selected_path=selected_path)
                ...
                next_weight = weight * fraction(selected_path["probability"])
                row = {**event, "state": branch["kind"], "selected_action_execution": selected["execution_result"],
                       "selected_action_path": selected_path}
                if isinstance(selected_path.get("action_leaf"), Mapping): row["attack_leaf"] = selected_path["action_leaf"]
                final_hp = selected_path["final_hp"]
                if index == 1:
                ...
                leaf = selected_path.get("action_leaf")
                if isinstance(leaf, Mapping):
                    intermediate = materialize_detached_predictive_intermediate_state(strategy_d0=view["strategy_d0"], terminal_leaf=leaf)
                    ...
                    if flinch:
                        finish(plan, next_weight, [*events, row, {"state": "cancelled_due_to_flinch", "actor": deepcopy(target), "action_id": lineup[1][1]["action_id"]}], final_hp); continue
                    pending_metadata = {**deepcopy(lineup[1][2]), "session_id": view["strategy_d0"]["session_id"],
                        "source_runtime_fingerprint": view["strategy_d0"]["source_runtime_fingerprint"], "source_branch_fingerprint": view["strategy_d0"]["strategy_preview_fingerprint"], "decision_owner": deepcopy(actor)}
                    pending = freeze_detached_intermediate_predictive_authority(strategy_d0=view["strategy_d0"], runtime_snapshot=view["runtime_snapshot"],
                    ...
                    walk(plan, lineup, 1, selected_path["post_action_runtime_snapshot"], next_weight, [*events, row], final_hp)
    ...
    try:
        ...
        for path in paths:
            ...
            for index, event in enumerate(path["actions"]):
                ...
                if branch["executes"]:
                    if not isinstance(selected, Mapping) or selected.get("status") != "resolved" or selected.get("actor") != actor or selected.get("action_id") != event["action_id"] or selected.get("move_id") != gate["move_id"]: raise ValueError()
