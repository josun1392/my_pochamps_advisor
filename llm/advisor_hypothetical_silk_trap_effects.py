...
def project_burning_bulwark_protection(*,branch_state,action,owner,success_authority):
 meta=canonical_burning_bulwark_reactive_burn_metadata(action.get("move",{}).get("move_id") if isinstance(action,Mapping) else None)
 if meta is None:return {"status":"unsupported","reason":"canonical_burning_bulwark_metadata"}
 proxy={"owner":deepcopy(dict(owner)),"move":{"move_id":"protect","category":action["move"].get("category"),"target":action["move"].get("target"),"accuracy":action["move"].get("accuracy")}}
 result=project_self_protection(branch_state=branch_state,action=proxy,expected_owner=owner,success_authority=success_authority)
 if result.get("status")!="resolved":return result
 return {**result,"metadata":meta,"provenance":"canonical_burning_bulwark_shared_ordinary_protection_v1"}
