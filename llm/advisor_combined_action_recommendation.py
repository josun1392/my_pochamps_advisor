...
def _switch_actions(evidence: Mapping[str, Any], snapshot: Any) -> list[dict[str, Any]]:
    ...
    for candidate in evidence.get("switch_candidates", []):
        ...
        entry_hazard_result = evaluate_switch_entry_effects(hazards=hazards, target=target, intimidate_authority=intimidate_authority, download_authority=download_authority) if isinstance(target, Mapping) else None
