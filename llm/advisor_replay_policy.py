...
_EFFECTS.update({"confusion_self_hit_disguise_broken_derived":"break_confusion_self_hit_disguise"})
...
def build_replay_plan(base_state, ordered_observations, *, canonical_move_resolver=None):
    ...
    damage_sources={e.get("observation_id"):e for e in evidence if e.get("event_kind")=="confusion_self_hit_damage_observed"}
    damage_verified=[]
    for event in accepted:
        if event.get("event_kind") in CONFUSION_SELF_HIT_DAMAGE_DERIVED_KINDS:
            source=damage_sources.get(event.get("payload",{}).get("source_damage_observation_id"))
            if (source is None or source.get("session_id")!=event.get("session_id")
                    or source.get("turn_number")!=event.get("turn_number")
                    or source.get("observation_sequence",0)>=event.get("observation_sequence",0)
                    or event.get("trust")!=CONFUSION_SELF_HIT_DAMAGE_DERIVED_TRUST
                    or event.get("source")!=CONFUSION_SELF_HIT_DAMAGE_DERIVED_SOURCE
                    or event.get("scope")!="confusion_self_hit_damage_lifecycle"):
                unsupported.append(event); conflicts.append({"observation_id":event.get("observation_id"),"reason":"invalid_confusion_self_hit_damage_derived_binding"}); continue
        damage_verified.append(event)
    accepted=damage_verified
    exact_executions = {}
    verified = []
    for event in accepted:
