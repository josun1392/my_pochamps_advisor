...
def _valid_same_turn_event(value, session_id):
    return isinstance(value, dict) and value.get("session_id") == session_id and value.get("predicate") in {"received_qualifying_direct_damage", "acted_earlier_this_turn", "lost_hp_this_turn", "qualifying_direct_damage_dealt"} and isinstance(value.get("occurred"), bool) and isinstance(value.get("turn_number"), int) and not isinstance(value.get("turn_number"), bool) and value["turn_number"] > 0 and all(isinstance(value.get(key), int) and not isinstance(value.get(key), bool) and value[key] >= 0 for key in ("slot_index", "target_slot_index")) and value.get("side") in {"self", "opponent"} and value.get("target_side") in {"self", "opponent"} and all(isinstance(value.get(key), str) and value[key] for key in ("pokemon_id", "target_pokemon_id")) and (value.get("side"), value.get("slot_index"), value.get("pokemon_id")) != (value.get("target_side"), value.get("target_slot_index"), value.get("target_pokemon_id")) and isinstance(value.get("provenance"), dict) and value["provenance"].get("event_kind") == "same_turn_event_observed" and value["provenance"].get("trust") == "user_confirmed_observation"
