"""Production admission for observed ordinary variable 2--5-hit results."""
...
from llm.advisor_variable_two_to_five_hit_graph_reconciliation import reconcile_observed_variable_two_to_five_hit_graph,validate_historical_variable_two_to_five_prediction
from llm.advisor_observation_runtime_session import BattleObservationRuntimeSessionManager

EXECUTION_SOURCE="ui_executed_move_confirmation"

def admit_observed_variable_two_to_five_hit_result(*,runtime_session_manager,captured_session_id,retained_prediction,turn_number,source_execution_observation,action_outcome,landed_hit_count,terminal_reason,ordered_hits):
 ...
 if not isinstance(runtime_session_manager,BattleObservationRuntimeSessionManager) or r["session_id"]!=captured_session_id or r["turn_number"]!=turn_number:return _out("rejected","variable_two_to_five_stale_session_or_turn")
 ...
 if action_outcome=="miss":
  if landed_hit_count!=0 or terminal_reason!="action_miss" or hits:return _out("rejected","variable_two_to_five_miss_observation_invalid")
 elif action_outcome=="landed":
  if landed_hit_count not in {1,2,3,4,5} or len(hits)!=landed_hit_count:return _out("rejected","variable_two_to_five_landed_count_invalid")
 else:return _out("rejected","variable_two_to_five_result_request_invalid")
 snap=runtime_session_manager.capture_runtime_state_snapshot(captured_session_id)
 if snap.get("status")!="runtime_snapshot_ready":return _out("rejected","variable_two_to_five_runtime_snapshot_unavailable")
 state=snap.get("state")
 if not _active(state,r["actor"]) or not _active(state,r["target"]):return _out("rejected","variable_two_to_five_actor_or_target_stale")
 ...
 if not executions or max(executions,key=lambda x:x.get("observation_sequence",0)).get("observation_id")!=source_execution_observation.get("observation_id"):return _out("rejected","variable_two_to_five_execution_observation_stale")
 pid=f"{captured_session_id}:variable-two-to-five:{turn_number}:{r['action_id']}:result"
 ...
 if old is not None:
  ...
  if old.get("payload")!=parent_payload or len(children)!=len(hits) or any(c.get("payload")!=_child_payload(r,pid,h) for c,h in zip(children,hits)):return _out("rejected","conflicting_variable_two_to_five_observation")
  rec=reconcile_observed_variable_two_to_five_hit_graph(retained_prediction=r,source_execution_observation=source_execution_observation,parent_observation=old,hit_observations=children,related_observations=coll.get("ordered_observations",()))
 ...
 if parent.get("status")!="confirmed":return _out("rejected","variable_two_to_five_parent_confirmation_rejected")
 ...
 if hits and current_hp!=hits[0]["hp_before"]:return _out("rejected","variable_two_to_five_pre_hp_runtime_mismatch")
 for hit in hits:
  ...
  if child.get("status")!="confirmed":return _out("rejected","variable_two_to_five_child_confirmation_rejected")
  _seq(runtime_session_manager,child);confirmations.append(child);children.append(child["observation"])
 rec=reconcile_observed_variable_two_to_five_hit_graph(retained_prediction=r,source_execution_observation=source_execution_observation,parent_observation=parent["observation"],hit_observations=children,related_observations=coll.get("ordered_observations",()))
 if rec.get("status")!="resolved" or rec.get("match_outcome")=="incompatible_observation":return {"status":"rejected","reason":"incompatible_variable_two_to_five_observation","idempotent":False,"parent_observation":parent["observation"],"hit_observations":tuple(children),"reconciliation":rec,"runtime_snapshot":snap}
 for child,hit in zip(children,hits):
  ...
  if hit["hp_after"]==0:
   ...
   if pair is None:return _out("rejected","variable_two_to_five_hp_faint_confirmation_rejected")
   ...
   if hp.get("status")!="confirmed":return _out("rejected","variable_two_to_five_hp_confirmation_rejected")
 ...
 if runtime_session_manager.preview(captured_session_id,preview).get("status")!="preview_ready":return _out("rejected","variable_two_to_five_preview_rejected")
 if runtime_session_manager.admit_confirmations_atomically(captured_session_id,confirmations).get("status") not in {"added","duplicate"}:return _out("rejected","variable_two_to_five_admission_rejected")
 if runtime_session_manager.apply(captured_session_id,runtime_session_manager.read_collection_snapshot()).get("status") not in {"applied","already_applied"}:return _out("rejected","variable_two_to_five_runtime_application_rejected")
 committed=runtime_session_manager.capture_runtime_state_snapshot(captured_session_id)
 if hits and _pokemon(committed.get("state"),target).get("current_hp")!=hits[-1]["hp_after"]:return _out("rejected","variable_two_to_five_committed_hp_mismatch")
...
def _execution_error(obs,r):
 ...
 if not isinstance(obs,Mapping) or obs.get("event_kind")!="executed_move_observed" or obs.get("source")!=EXECUTION_SOURCE or obs.get("trust")!=USER_TRUST or obs.get("session_id")!=r["session_id"] or obs.get("turn_number")!=r["turn_number"] or (obs.get("side"),obs.get("slot_index"),obs.get("pokemon_id"))!=(a["side"],a["slot_index"],a["pokemon_id"]):return "variable_two_to_five_execution_observation_mismatch"
 if not isinstance(p,Mapping) or p.get("move_id")!=r["move_id"] or p.get("source_action_id")!=r["source_action_id"]:return "variable_two_to_five_execution_payload_mismatch"
 return None

def _normalize_hits(rows,count):
 if not isinstance(rows,(tuple,list)) or len(rows)!=count:return "variable_two_to_five_ordered_hit_count_invalid"
 out=[]
 for i,x in enumerate(rows,1):
  if not isinstance(x,Mapping) or x.get("hit_index")!=i:return "variable_two_to_five_ordered_hit_index_invalid"
  b,a=x.get("hp_before"),x.get("hp_after")
  if not _nn(b) or not _nn(a) or a>b:return "variable_two_to_five_ordered_hit_hp_invalid"
  crit=x.get("critical_state")
  if crit not in {None,"critical","non_critical"}:return "variable_two_to_five_critical_observation_invalid"
  ids=x.get("related_contact_observation_ids",())
  if not isinstance(ids,(tuple,list)) or len(ids)!=len(set(ids)) or any(not isinstance(v,str) or not v for v in ids):return "variable_two_to_five_contact_links_invalid"
 ...
 if any(out[i]["hp_after"]!=out[i+1]["hp_before"] for i in range(len(out)-1)):return "variable_two_to_five_ordered_hp_chain_invalid"
 return tuple(out)

def _parent_payload(r,e,o,c,t):return {"family":"variable_two_to_five","decision_point":r["decision_point"],"action_id":r["action_id"],"move_id":r["move_id"],"actor":deepcopy(r["actor"]),"target":deepcopy(r["target"]),"action_outcome":o,"landed_hit_count":c,"terminal_reason":t,"source_execution_observation_id":e["observation_id"],"predictive_artifact_fingerprint":r["predictive_artifact_fingerprint"]}
def _child_payload(r,p,h):return {"family":"variable_two_to_five","decision_point":r["decision_point"],"action_id":r["action_id"],"move_id":r["move_id"],"actor":deepcopy(r["actor"]),"target":deepcopy(r["target"]),"parent_multi_hit_observation_id":p,**deepcopy(dict(h))}
def _seq(m,x):
