...
def _knock_off_power_context(stat_provenance: Mapping[str, Any], current: Mapping[str, Any]) -> dict[str, Any]:
    defender = _mapping(stat_provenance.get("defender"))
    result = resolve_knock_off_target_item(
        item_authority=_mapping(defender.get("known_item")),
...
def _fling_power_context(current: Mapping[str, Any], move: Mapping[str, Any]) -> dict[str, Any]:
    ...
    canonical_persim_berry = resolve_canonical_fling_persim_confusion_cure_berry(item_id)
    persim_confusion_cure = (
        effect.get("kind") == "berry_effect"
        and authority.get("fling_persim_confusion_cure_berry_support")
        == "fling_persim_confusion_cure_target_effect_v1"
        and canonical_persim_berry.get("status") == "resolved"
        and authority.get("fling_persim_confusion_cure_berry_authority")
        == canonical_persim_berry
    )
    canonical_lum_berry = resolve_canonical_fling_lum_major_status_confusion_cure_berry(item_id)
    lum_major_status_confusion_cure = (
        effect.get("kind") == "berry_effect"
        and authority.get("fling_lum_major_status_confusion_cure_support")
        == "fling_lum_major_status_confusion_cure_target_effect_v1"
        and canonical_lum_berry.get("status") == "resolved"
        and authority.get("fling_lum_major_status_confusion_cure_berry_authority")
        == canonical_lum_berry
    )
    canonical_hp_restore_berry = resolve_canonical_fling_hp_restore_berry(item_id)
    hp_restore_berry = (
        effect.get("kind") == "berry_effect"
        and authority.get("fling_hp_restore_berry_support")
        == "fling_hp_restore_berry_target_effect_v1"
        and canonical_hp_restore_berry.get("status") == "resolved"
    ...
    return {
        ...
        "target_cure_conditions": (
            tuple(canonical_berry.get("removable_conditions", ()))
            if berry_cure
            else tuple(canonical_lum_berry.get("removable_conditions", ()))
            if lum_major_status_confusion_cure
            else ()
        ),
        "target_condition_context_values": (
            ("burn", "poison", "toxic", "paralysis", "sleep", "freeze")
            if berry_cure or lum_major_status_confusion_cure
            else ()
        ),
