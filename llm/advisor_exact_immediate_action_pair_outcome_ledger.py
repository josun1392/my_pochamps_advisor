...
from llm.advisor_runtime_d0_fling_persim_confusion_cure_target_effect_authority import (
    validate_detached_fling_persim_confusion_cure_target_effect,
)
...
def _leaf(value: Any, base: Mapping[str, Any]) -> dict[str, Any] | str:
    ...
    fling_type_resist_error = _fling_type_resist_empty_intrinsic_berry_target_effect_leaf(first)
    if fling_type_resist_error is not None: return fling_type_resist_error
    fling_persim_error = _fling_persim_confusion_cure_target_effect_leaf(first)
    ...
    fling_leppa_error = _fling_leppa_pp_restore_target_effect_leaf(first)
    if fling_leppa_error is not None: return fling_leppa_error
    ...
    if isinstance(second_leaf, Mapping):
        ...
        if fling_berry_cure_error is not None: return fling_berry_cure_error
        fling_type_resist_error = _fling_type_resist_empty_intrinsic_berry_target_effect_leaf(second_leaf)
        if fling_type_resist_error is not None: return fling_type_resist_error
        fling_persim_error = _fling_persim_confusion_cure_target_effect_leaf(second_leaf)
        if fling_persim_error is not None: return fling_persim_error
        ...
        if fling_hp_restore_error is not None: return fling_hp_restore_error
        fling_leppa_error = _fling_leppa_pp_restore_target_effect_leaf(second_leaf)
        if fling_leppa_error is not None: return fling_leppa_error
...
def _fling_item_throw_leaf(leaf: Mapping[str, Any]) -> str | None:
    ...
    canonical_type_resist = (
        resolve_canonical_fling_type_resist_empty_intrinsic_berry(item.get("value"))
        if isinstance(item, Mapping) else {"status": "not_applicable"}
    )
    type_resist_supported = (
        authority.get("fling_type_resist_empty_intrinsic_berry_support")
        == "fling_type_resist_empty_intrinsic_berry_target_effect_v1"
        and canonical_type_resist.get("status") == "resolved"
        and authority.get("fling_type_resist_empty_intrinsic_berry_authority")
        == canonical_type_resist
        and effect.get("kind") == "berry_effect"
    )
    canonical_persim = (
        resolve_canonical_fling_persim_confusion_cure_berry(item.get("value"))
        if isinstance(item, Mapping) else {"status": "not_applicable"}
    )
    persim_supported = (
        authority.get("fling_persim_confusion_cure_berry_support")
        == "fling_persim_confusion_cure_target_effect_v1"
        and canonical_persim.get("status") == "resolved"
        and authority.get("fling_persim_confusion_cure_berry_authority")
        == canonical_persim
        and effect.get("kind") == "berry_effect"
    )
    canonical_lum = (
        resolve_canonical_fling_lum_major_status_confusion_cure_berry(item.get("value"))
        if isinstance(item, Mapping) else {"status": "not_applicable"}
    )
    lum_supported = (
        authority.get("fling_lum_major_status_confusion_cure_support")
        == "fling_lum_major_status_confusion_cure_target_effect_v1"
        and canonical_lum.get("status") == "resolved"
        and authority.get("fling_lum_major_status_confusion_cure_berry_authority")
        == canonical_lum
        and effect.get("kind") == "berry_effect"
    )
...
def _fling_item_bound_target_effect_leaf(leaf: Mapping[str, Any]) -> str | None:
    ...
    if kind=="major_status":
        if outcome=="applied_major_status" and not isinstance(effect.get("hypothetical_target_condition"),Mapping):return "fling_target_effect_status_materialization_invalid"
        if outcome not in {"applied_major_status","no_transition_already_statused","prevented","not_applicable"}:return "fling_target_effect_status_outcome_invalid"
    elif kind=="flinch":
...
def _fling_berry_eaten_transition_leaf(leaf: Mapping[str, Any]) -> str | None:
    ...
    leppa = consequences.get("fling_leppa_pp_restore_target_effect") if isinstance(consequences, Mapping) else None
    move = provenance.get("move_id") if isinstance(provenance, Mapping) else None
    if transition is not None and move != "fling":
        return "unexpected_fling_berry_eaten_transition"
    if move != "fling":
        return None
    cure_interaction = cure.get("authority", {}).get("berry_eat_item_interaction_authority") if isinstance(cure, Mapping) else None
    ...
    leppa_interaction = leppa.get("authority", {}).get("berry_eat_item_interaction_authority") if isinstance(leppa, Mapping) else None
    interactions = [
        value for value in (cure_interaction, type_resist_interaction, persim_interaction, lum_interaction, hp_restore_interaction, leppa_interaction)
...
def _fling_type_resist_empty_intrinsic_berry_target_effect_leaf(
    ...
    provenance, consequences = leaf.get("provenance"), leaf.get("consequences")
    payload = (
        consequences.get("fling_type_resist_empty_intrinsic_berry_target_effect")
        if isinstance(consequences, Mapping) else None
    )
    transition = (
        consequences.get("fling_berry_eaten_transition")
        if isinstance(consequences, Mapping) else None
    )
    move = provenance.get("move_id") if isinstance(provenance, Mapping) else None
    if move != "fling":
        return (
            "unexpected_fling_type_resist_empty_intrinsic_berry_payload"
            if payload is not None else None
        )
    execution = (
        provenance.get("fling_execution_authority")
        if isinstance(provenance, Mapping) else None
    )
    if not isinstance(execution, Mapping):
        return "fling_type_resist_berry_execution_authority_missing"
    supported = (
        execution.get("fling_type_resist_empty_intrinsic_berry_support")
        == "fling_type_resist_empty_intrinsic_berry_target_effect_v1"
    )
    if not supported:
        return (
    ...
    if execution.get("fling_major_status_cure_berry_support") is not None:
        return "fling_berry_family_support_overlap"

    item_id = execution.get("user_item_before", {}).get("value")
    canonical = resolve_canonical_fling_type_resist_empty_intrinsic_berry(item_id)
    if (
        canonical.get("status") != "resolved"
        or execution.get("fling_type_resist_empty_intrinsic_berry_authority")
        != canonical
        or execution.get("resolved_base_power") != 10
        or execution.get("fling_item_metadata") != canonical.get("fling_item_metadata")
    ):
        return "fling_type_resist_berry_family_binding_invalid"
    ...
    if not successful_eat:
        if payload is not None:
            return "fling_type_resist_berry_payload_without_authenticated_eat"
        if transition is not None:
            return "fling_type_resist_berry_transition_without_authenticated_eat"
        return None
    ...
    if (
        ...
        or not validate_detached_fling_type_resist_empty_intrinsic_berry_target_effect(
            consequence=payload,
            source_leaf=leaf,
            expected_target=target,
        )
    ):
        return "fling_type_resist_berry_target_effect_invalid"
    ...
    if (
        ...
        or authority.get("item_id") != item_id
    ...
    if not validate_detached_berry_eaten_transition(
        ...
        return "fling_type_resist_berry_ateberry_transition_invalid"
    return None


def _fling_persim_confusion_cure_target_effect_leaf(
    leaf: Mapping[str, Any],
) -> str | None:
    provenance, consequences = leaf.get("provenance"), leaf.get("consequences")
    payload = (
        consequences.get("fling_persim_confusion_cure_target_effect")
        if isinstance(consequences, Mapping) else None
    )
    transition = (
        consequences.get("fling_berry_eaten_transition")
    ...
    move = provenance.get("move_id") if isinstance(provenance, Mapping) else None
    if move != "fling":
        return "unexpected_fling_persim_payload" if payload is not None else None
    execution = (
        provenance.get("fling_execution_authority")
        if isinstance(provenance, Mapping) else None
    )
    if not isinstance(execution, Mapping):
        return "fling_persim_execution_authority_missing"
    supported = (
        execution.get("fling_persim_confusion_cure_berry_support")
        == "fling_persim_confusion_cure_target_effect_v1"
    )
    if not supported:
        return "unexpected_fling_persim_payload" if payload is not None else None
    if (
        execution.get("fling_major_status_cure_berry_support") is not None
        or execution.get("fling_type_resist_empty_intrinsic_berry_support") is not None
    ):
        return "fling_berry_family_support_overlap"
    canonical = resolve_canonical_fling_persim_confusion_cure_berry(
        execution.get("user_item_before", {}).get("value")
    )
    if (
        canonical.get("status") != "resolved"
        or execution.get("fling_persim_confusion_cure_berry_authority") != canonical
        or execution.get("resolved_base_power") != 10
        or execution.get("fling_item_metadata") != canonical.get("fling_item_metadata")
    ):
        return "fling_persim_family_binding_invalid"

    source_hit = (
        consequences.get("source_hit_context")
        if isinstance(consequences, Mapping) else None
    )
    target_hp = consequences.get("target_final_hp") if isinstance(consequences, Mapping) else None
    target_ko = consequences.get("target_ko") if isinstance(consequences, Mapping) else None
    ...
    if not successful_eat:
        if payload is not None:
            return "fling_persim_payload_without_authenticated_eat"
        if transition is not None:
            return "fling_persim_transition_without_authenticated_eat"
        return None
    ...
    if (
        ...
        or authority.get("actor") != provenance.get("attacker")
        or authority.get("target") != target
        or authority.get("item_id") != "persim-berry"
        or authority.get("action_id") != leaf.get("candidate_id")
        or authority.get("source_leaf_id") != leaf.get("leaf_id")
        or any(
            authority.get(key) != provenance.get(key)
    ...
    if not validate_detached_berry_eaten_transition(
        ...
        return "fling_persim_ateberry_transition_invalid"
    return None


def _fling_lum_major_status_confusion_cure_target_effect_leaf(
    leaf: Mapping[str, Any],
) -> str | None:
    provenance, consequences = leaf.get("provenance"), leaf.get("consequences")
    payload = (
        consequences.get("fling_lum_major_status_confusion_cure_target_effect")
        if isinstance(consequences, Mapping) else None
    )
    transition = (
        consequences.get("fling_berry_eaten_transition")
    ...
    move = provenance.get("move_id") if isinstance(provenance, Mapping) else None
    if move != "fling":
        return "unexpected_fling_lum_payload" if payload is not None else None
    execution = provenance.get("fling_execution_authority") if isinstance(provenance, Mapping) else None
    if not isinstance(execution, Mapping):
        return "fling_lum_execution_authority_missing"
    supported = (
        execution.get("fling_lum_major_status_confusion_cure_support")
        == "fling_lum_major_status_confusion_cure_target_effect_v1"
    )
    if not supported:
        return "unexpected_fling_lum_payload" if payload is not None else None
    if any(
        ...
        return "fling_berry_family_support_overlap"
    canonical = resolve_canonical_fling_lum_major_status_confusion_cure_berry(
        execution.get("user_item_before", {}).get("value")
    )
    if (
        canonical.get("status") != "resolved"
        ...
        return "fling_lum_family_binding_invalid"
    ...
    if not successful_eat:
        if payload is not None:
            return "fling_lum_payload_without_authenticated_eat"
        if transition is not None:
            return "fling_lum_transition_without_authenticated_eat"
        return None
...
def _fling_leppa_pp_restore_target_effect_leaf(leaf: Mapping[str, Any]) -> str | None:
    provenance, consequences = leaf.get("provenance"), leaf.get("consequences")
    payload = consequences.get("fling_leppa_pp_restore_target_effect") if isinstance(consequences, Mapping) else None
    transition = consequences.get("fling_berry_eaten_transition") if isinstance(consequences, Mapping) else None
    move = provenance.get("move_id") if isinstance(provenance, Mapping) else None
    if move != "fling":
        return "unexpected_fling_leppa_payload" if payload is not None else None
    execution = provenance.get("fling_execution_authority") if isinstance(provenance, Mapping) else None
    if not isinstance(execution, Mapping):
        return "fling_leppa_execution_authority_missing"
    supported = (
        execution.get("fling_leppa_pp_restore_support")
        == "fling_leppa_pp_restore_target_effect_v1"
    )
    if not supported:
        return "unexpected_fling_leppa_payload" if payload is not None else None
    ...
    if not successful_eat:
        if payload is not None:
            return "fling_leppa_payload_without_authenticated_eat"
        if transition is not None:
            return "fling_leppa_transition_without_authenticated_eat"
        return None
    target = provenance.get("target") if isinstance(provenance, Mapping) else None
    if not isinstance(target, Mapping) or not isinstance(payload, Mapping):
        return "fling_leppa_target_effect_missing"
    if not validate_detached_fling_leppa_pp_restore_target_effect(
        consequence=payload,
        source_leaf=leaf,
        expected_target=target,
    ):
        return "fling_leppa_target_effect_invalid"
    authority = payload.get("authority")
    if not validate_fling_leppa_pp_restore_target_effect_authority(authority):
        return "fling_leppa_target_effect_authority_invalid"
    if (
        authority.get("fling_execution_authority") != execution
        or authority.get("actor") != provenance.get("attacker")
        or authority.get("target") != target
        ...
        return "fling_leppa_target_effect_binding_invalid"
    if not validate_detached_berry_eaten_transition(
        transition, leaf=leaf, target=target,
    ):
        return "fling_leppa_ateberry_transition_invalid"
    return None
...
def _reflected_taunt_leaf(leaf: Mapping[str, Any], base: Mapping[str, Any]) -> str | None:
    ...
    reflected_application_marker = (
        ...
        and consequence_application.get("move_id") == "taunt"
    ) or (
        isinstance(provenance_application, Mapping)
        and provenance_application.get("execution_mode") == "reflected_original_action"
        and provenance_application.get("move_id") == "taunt"
    )
    reflected_path_marker = isinstance(leaf.get("branch_path"), (tuple, list)) and tuple(leaf["branch_path"][:2]) == ("taunt", "reflected")
    route_marker = (isinstance(consequence_route, Mapping) and consequence_route.get("move_id") == "taunt") or (isinstance(provenance_route, Mapping) and provenance_route.get("move_id") == "taunt")
    marker = route_marker or reflected_application_marker or reflected_path_marker
...
def _reflected_taunt_branch_binding(first: Mapping[str, Any], second_leaf: Any, base: Mapping[str, Any]) -> str | None:
    def route_of(leaf: Any) -> Mapping[str, Any] | None:
        ...
        return route if isinstance(route, Mapping) and route.get("move_id") == "taunt" else None
...
def _reflected_disable_leaf(leaf: Mapping[str, Any], base: Mapping[str, Any]) -> str | None:
    consequences=leaf.get("consequences")
    provenance=leaf.get("provenance")
    consequence_route=consequences.get("reflected_status_routing_authority") if isinstance(consequences,Mapping) else None
    provenance_route=provenance.get("reflected_status_routing_authority") if isinstance(provenance,Mapping) else None
    consequence_application=consequences.get("disable_application") if isinstance(consequences,Mapping) else None
    provenance_application=provenance.get("disable_application") if isinstance(provenance,Mapping) else None
    reflected_application_marker=(
        isinstance(consequence_application,Mapping)
        and consequence_application.get("execution_mode")=="reflected_original_action"
        and consequence_application.get("move_id")=="disable"
    ) or (
        isinstance(provenance_application,Mapping)
        and provenance_application.get("execution_mode")=="reflected_original_action"
        and provenance_application.get("move_id")=="disable"
    )
    reflected_path_marker=isinstance(leaf.get("branch_path"),(tuple,list)) and tuple(leaf["branch_path"][:2])==("disable","reflected")
    route_marker=(isinstance(consequence_route,Mapping) and consequence_route.get("move_id")=="disable") or (isinstance(provenance_route,Mapping) and provenance_route.get("move_id")=="disable")
    if not (route_marker or reflected_application_marker or reflected_path_marker):return None
    if not isinstance(consequence_route,Mapping) or consequence_route!=provenance_route:return "reflected_disable_ledger_route_mismatch"
    if not isinstance(consequence_application,Mapping) or consequence_application!=provenance_application:return "reflected_disable_ledger_application_mismatch"
    try:
        from llm.advisor_detached_reflected_status_routing_authority import validate_detached_reflected_status_routing_authority
        from llm.advisor_detached_disable_action_restriction import materialize_detached_reflected_disable_application
        validation=validate_detached_reflected_status_routing_authority(consequence_route)
        if validation.get("status")!="resolved":return "reflected_disable_ledger_route_invalid"
        route=validation["authority"]
        candidate_id=leaf.get("candidate_id")
        if candidate_id==base["own_action_id"]:selected_actor,selected_target=base["own_actor"],base["opponent_actor"]
        elif candidate_id==base["opponent_action_id"]:selected_actor,selected_target=base["opponent_actor"],base["own_actor"]
        else:return "reflected_disable_ledger_selected_action_invalid"
        if (
            route.get("session_id")!=base["session_id"]
            or route.get("source_runtime_fingerprint")!=base["source_runtime_fingerprint"]
            or route.get("source_branch_fingerprint")!=base["source_branch_fingerprint"]
            ...
            or route.get("reflected_source")!=selected_target
            or route.get("reflected_target")!=selected_actor
        ):return "reflected_disable_ledger_selected_route_binding_invalid"
        if (
            consequence_application.get("status")!="resolved"
            or consequence_application.get("execution_mode")!="reflected_original_action"
            or consequence_application.get("action_id")!=candidate_id
            or consequence_application.get("move_id")!="disable"
            or consequence_application.get("actor")!=selected_target
            or consequence_application.get("target")!=selected_actor
            or consequence_application.get("reflected_status_routing_authority")!=route
        ):return "reflected_disable_ledger_application_binding_invalid"
        if (
            provenance.get("attacker")!=selected_actor
            or provenance.get("target")!=selected_target
            or provenance.get("selected_actor")!=selected_actor
            ...
            or provenance.get("effective_source")!=selected_target
            or provenance.get("effective_target")!=selected_actor
            or provenance.get("move_id")!="disable"
        ):return "reflected_disable_ledger_leaf_identity_invalid"
        if (
            leaf.get("hit_state")!="not_applicable"
            or leaf.get("critical_state")!="not_applicable"
            or leaf.get("damage_roll")!="not_applicable"
            ...
            or consequences.get("contact")!="not_applicable"
            or _fraction(leaf.get("probability"))!=Fraction(1,1)
        ):return "reflected_disable_ledger_zero_damage_identity_invalid"
        request=route.get("validation_request")
        if not isinstance(request,Mapping):return "reflected_disable_ledger_route_request_missing"
        reproduced=materialize_detached_reflected_disable_application(strategy_d0=request["strategy_d0"],runtime_snapshot=request["runtime_snapshot"],action=request["action"],routing_authority=route)
        if reproduced!=consequence_application:return "reflected_disable_ledger_application_reproduction_invalid"
        if consequence_application.get("outcome")=="applicable":
            if (
                consequences.get("disabled_move_id")!=consequence_application.get("disabled_move_id")
            ...
            ):return "reflected_disable_ledger_family_consequence_invalid"
        active=request["strategy_d0"].get("strategy_state",{}).get("active",{})
        selected_hp=active.get(selected_actor["side"],{}).get("current_hp")
        target_hp=active.get(selected_target["side"],{}).get("current_hp")
        if (
            ...
            or consequences.get("self_fainted")!=(selected_hp==0)
            or consequences.get("target_ko")!=(target_hp==0)
        ):return "reflected_disable_ledger_hp_orientation_invalid"
    except (KeyError,TypeError,ValueError,ZeroDivisionError):
        return "reflected_disable_ledger_validation_error"
    return None


def _reflected_disable_branch_binding(first: Mapping[str, Any], second_leaf: Any, base: Mapping[str, Any]) -> str | None:
    def route_of(leaf: Any) -> Mapping[str, Any] | None:
        consequences=leaf.get("consequences") if isinstance(leaf,Mapping) else None
        route=consequences.get("reflected_status_routing_authority") if isinstance(consequences,Mapping) else None
        return route if isinstance(route,Mapping) and route.get("move_id")=="disable" else None
    first_route,second_route=route_of(first),route_of(second_leaf)
    if first_route is not None and second_route is not None:return "reflected_disable_ledger_multiple_reflected_actions_invalid"
    if first_route is None and second_route is None:return None
    reflected_leaf,other_leaf,route=(first,second_leaf,first_route) if first_route is not None else (second_leaf,first,second_route)
    if not isinstance(reflected_leaf,Mapping) or not isinstance(other_leaf,Mapping) or not isinstance(route,Mapping):return "reflected_disable_ledger_pair_action_missing"
    selected_id=route.get("original_action_id")
    if selected_id==base["own_action_id"]:pending_id,pending_actor=base["opponent_action_id"],base["opponent_actor"]
    elif selected_id==base["opponent_action_id"]:pending_id,pending_actor=base["own_action_id"],base["own_actor"]
    else:return "reflected_disable_ledger_selected_action_invalid"
    other_provenance=other_leaf.get("provenance")
    if other_leaf.get("candidate_id")!=pending_id or not isinstance(other_provenance,Mapping) or other_provenance.get("attacker")!=pending_actor or pending_actor!=route.get("original_target"):
        return "reflected_disable_ledger_pending_action_binding_invalid"
    other_consequences=other_leaf.get("consequences")
    if isinstance(other_consequences,Mapping) and (
        other_consequences.get("execution_failure")=="disable_action_restriction"
        or "disable_execution_gate" in other_consequences
    ):return "reflected_disable_ledger_wrong_pending_disable_cancellation"
    return None
