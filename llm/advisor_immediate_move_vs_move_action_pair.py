...
def _gated_extension_authorities(**authorities: Any) -> dict[str, Any]:
    """Pass only existing sparse family evidence into a gate handoff.

    Gate owners decide opportunity.  They must not derive a neutral authority
    from a missing map, so absent values are omitted rather than defaulted.
    """
    return {key: deepcopy(value) for key, value in authorities.items() if value is not None}
...
def _materialize_order(
    *, strategy_d0: Mapping[str, Any], runtime_snapshot: Mapping[str, Any],
    ...
    if order == "opponent_first" and own_meta["metadata"].get("move_id") == "fling":
        leppa_execution = freeze_runtime_d0_fling_item_execution_authority(
            strategy_d0=strategy_d0,
            runtime_snapshot=runtime_snapshot,
            action=own_action,
            actor=base["own_actor"],
            target=base["opponent_actor"],
        )
        if leppa_execution.get("fling_leppa_pp_restore_support") == "fling_leppa_pp_restore_target_effect_v1":
            return _result(
                "incomplete",
                "fling_leppa_target_first_post_action_pp_authority_unavailable",
                base,
            )
...
def _attack_ledger_before_ability_steal(*, strategy_d0: Mapping[str, Any], runtime_snapshot: Mapping[str, Any], actor: Mapping[str, Any], target: Mapping[str, Any], metadata_authority: Mapping[str, Any], sturdy_survival_authority: Mapping[str, Any] | None = None, focus_sash_survival_authority: Mapping[str, Any] | None = None, endure_turn_survival_authority: Mapping[str, Any] | None = None, action: Mapping[str, Any] | None = None, analytic_action_order_authority: Mapping[str, Any] | None = None, stakeout_switch_authority: Mapping[str, Any] | None = None, same_turn_last_incoming_attack_event: Mapping[str, Any] | None = None, post_source_retaliation_protection_authority: Mapping[str, Any] | None = None, source_terminal_leaf: Mapping[str, Any] | None = None, source_selected_action: Mapping[str, Any] | None = None, source_execution_order_provenance: Mapping[str, Any] | None = None, pending_target_action: Mapping[str, Any] | None = None, action_order: str | None = None, semi_invulnerable_exception_damage_modifier_authority: Mapping[str, Any] | None = None, semi_invulnerable_targetability_authority: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if fling_execution is not None:
        ...
        hp_restore = _apply_fling_hp_restore_berry_target_effect_to_ledger(
            ledger=lum,
            strategy_d0=strategy_d0,
            runtime_snapshot=runtime_snapshot,
            authority=fling_execution,
        ...
        return _apply_fling_leppa_pp_restore_target_effect_to_ledger(
            ledger=hp_restore,
            strategy_d0=strategy_d0,
            runtime_snapshot=runtime_snapshot,
            authority=fling_execution,
...
def _apply_fling_leppa_pp_restore_target_effect_to_ledger(
    *,
    ledger: Mapping[str, Any],
    strategy_d0: Mapping[str, Any],
    runtime_snapshot: Mapping[str, Any],
    ...
    supported = (
        authority.get("fling_leppa_pp_restore_support")
    ...
    for leaf in ledger["terminal_leaves"]:
        ...
        bound = freeze_runtime_d0_fling_leppa_pp_restore_target_effect_authority(
            strategy_d0=strategy_d0,
            runtime_snapshot=runtime_snapshot,
            fling_execution_authority=authority,
            source_leaf=leaf,
        ...
        if bound.get("status") != "resolved":
            return _result(
                _status(bound),
                bound.get("reason", "fling_leppa_pp_restore_target_effect_unavailable"),
                {},
            )
        if bound.get("outcome") == "not_applicable":
            rows.append(deepcopy(dict(leaf)))
        ...
        if detached.get("status") != "resolved":
            return _result(
                _status(detached),
                detached.get("reason", "fling_leppa_pp_restore_materialization_unavailable"),
                {},
            )
        eaten = materialize_detached_fling_berry_eaten_transition(
            strategy_d0=strategy_d0,
