...
def _disable_pair_leaf(application: Mapping[str, Any], strategy_d0: Mapping[str, Any]) -> dict[str, Any] | str:
    active=strategy_d0.get("strategy_state",{}).get("active",{}); own=active.get(application.get("actor",{}).get("side"),{}).get("current_hp"); foe=active.get(application.get("target",{}).get("side"),{}).get("current_hp")
...
def _attack_ledger(*, strategy_d0: Mapping[str, Any], runtime_snapshot: Mapping[str, Any], actor: Mapping[str, Any], target: Mapping[str, Any], metadata_authority: Mapping[str, Any], sturdy_survival_authority: Mapping[str, Any] | None = None, focus_sash_survival_authority: Mapping[str, Any] | None = None, action: Mapping[str, Any] | None = None, analytic_action_order_authority: Mapping[str, Any] | None = None, stakeout_switch_authority: Mapping[str, Any] | None = None, same_turn_last_incoming_attack_event: Mapping[str, Any] | None = None, post_source_retaliation_protection_authority: Mapping[str, Any] | None = None, source_terminal_leaf: Mapping[str, Any] | None = None, source_selected_action: Mapping[str, Any] | None = None, source_execution_order_provenance: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    target_current_hp_power = None
    if metadata.get("move_id") == "fling":
        if not isinstance(action, Mapping): return _result("incomplete", "fling_action_authority_required", {})
        fling_execution = freeze_runtime_d0_fling_item_execution_authority(strategy_d0=strategy_d0, runtime_snapshot=runtime_snapshot, action=action, actor=actor, target=target)
        if fling_execution.get("status") != "resolved": return _result(_status(fling_execution), fling_execution.get("reason", "fling_execution_authority_unavailable"), {})
    ...
    if metadata.get("move_id") in {"hard-press", "crush-grip", "wring-out"}:
        target_current_hp_power = freeze_runtime_d0_target_current_hp_power_authority(strategy_d0=strategy_d0, runtime_snapshot=runtime_snapshot, move=metadata, user=actor, target=target)
        if target_current_hp_power.get("status") != "resolved": return _result(_status(target_current_hp_power), target_current_hp_power.get("reason", "target_current_hp_power_authority_unavailable"), {})
        metadata = {**metadata, "target_current_hp_power_authority": target_current_hp_power}
    ...
    if metadata.get("move_id") in {"avalanche", "revenge"}:
        power = materialize_detached_was_damaged_by_target_power_authority(strategy_d0=strategy_d0, move=metadata, user=actor, target=target, incoming_event=same_turn_last_incoming_attack_event, source_terminal_leaf=source_terminal_leaf, execution_order_provenance=source_execution_order_provenance)
    ...
    if metadata.get("move_id") == "payback":
        power = materialize_detached_target_already_acted_power_authority(strategy_d0=strategy_d0, move=metadata, user=actor, target=target, source_terminal_leaf=source_terminal_leaf, source_selected_action=source_selected_action, execution_order_provenance=source_execution_order_provenance)
        if power.get("status") != "resolved": return _result(_status(power), power.get("reason", "target_already_acted_power_authority_unavailable"), {})
        metadata = {**metadata, "target_already_acted_power_authority": power}
    if metadata.get("move_id") == "stomping-tantrum":
        previous = freeze_runtime_d0_previous_action_result_authority(strategy_d0=strategy_d0, runtime_snapshot=runtime_snapshot, owner=actor)
        power = materialize_previous_action_failure_power_authority(strategy_d0=strategy_d0, move=metadata, user=actor, previous_action_authority=previous)
        if power.get("status") != "resolved": return _result(_status(power), power.get("reason", "previous_action_result_authority_unavailable"), {})
        metadata = {**metadata, "previous_action_result_authority": power}
    if metadata.get("move_id") == "lash-out":
        power=materialize_detached_same_turn_stat_drop_power_authority(strategy_d0=strategy_d0,move=metadata,user=actor,source_terminal_leaf=source_terminal_leaf,execution_order_provenance=source_execution_order_provenance)
        if power.get("status")!="resolved": return _result(_status(power),power.get("reason","same_turn_stat_drop_power_authority_unavailable"),{})
        metadata={**metadata,"same_turn_stat_drop_power_authority":power}
    if metadata.get("move_id")=="rage-fist":
        count=freeze_runtime_d0_rage_fist_hit_count_authority(strategy_d0=strategy_d0,runtime_snapshot=runtime_snapshot,owner=actor)
        power=materialize_detached_rage_fist_hit_count_power_authority(strategy_d0=strategy_d0,move=metadata,user=actor,base_count_authority=count,source_terminal_leaf=source_terminal_leaf)
        if power.get("status")!="resolved":return _result(_status(power),power.get("reason","rage_fist_hit_count_authority_unavailable"),{})
        metadata={**metadata,"rage_fist_hit_count_power_authority":power}
    if metadata.get("move_id")=="last-respects":
        history=freeze_runtime_d0_last_respects_faint_history_authority(strategy_d0=strategy_d0,runtime_snapshot=runtime_snapshot,owner=actor)
        power=materialize_detached_last_respects_faint_power_authority(strategy_d0=strategy_d0,move=metadata,user=actor,history_authority=history)
        if power.get("status")!="resolved":return _result(_status(power),power.get("reason","last_respects_faint_history_unavailable"),{})
        metadata={**metadata,"last_respects_faint_power_authority":power}
    ...
    if metadata.get("move_id") == "stomping-tantrum": return _bind_previous_action_failure_power_authority_to_ledger(normal, power)
    if metadata.get("move_id") == "lash-out": return _bind_same_turn_stat_drop_power_authority_to_ledger(normal,power)
...
def _bind_target_current_hp_power_authority_to_ledger(ledger: Mapping[str, Any], authority: Mapping[str, Any]) -> dict[str, Any]:
    if ledger.get("status") != "evaluable" or not isinstance(ledger.get("terminal_leaves"), tuple): return deepcopy(dict(ledger))
    if authority.get("status") != "resolved" or authority.get("family") != "target_current_hp_power": return _result("rejected", "target_current_hp_power_ledger_authority_invalid", {})
    rows = []
    for leaf in ledger["terminal_leaves"]:
        if not isinstance(leaf, Mapping) or not isinstance(leaf.get("provenance"), Mapping): return _result("rejected", "target_current_hp_power_ledger_leaf_invalid", {})
        row = deepcopy(dict(leaf)); row["provenance"] = {**deepcopy(dict(row["provenance"])), "target_current_hp_power_authority": deepcopy(dict(authority))}; rows.append(row)
    result = deepcopy(dict(ledger)); result["terminal_leaves"] = tuple(rows)
    result["component_manifest"] = {**deepcopy(dict(result.get("component_manifest", {}))), "target_current_hp_power": {"status": "resolved"}}
    return result


def _bind_was_damaged_power_authority_to_ledger(ledger: Mapping[str, Any], authority: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(authority, Mapping) or authority.get("status") != "resolved" or authority.get("canonical_base_power") != 60 or authority.get("selected_base_power") not in {60, 120}:
        return _result("rejected", "was_damaged_power_ledger_authority_invalid", {})
    ...
    for leaf in ledger["terminal_leaves"]:
        if not isinstance(leaf, Mapping) or not isinstance(leaf.get("provenance"), Mapping):
            return _result("rejected", "was_damaged_power_ledger_leaf_invalid", {})
        row = deepcopy(dict(leaf))
        row["provenance"] = {**deepcopy(dict(row["provenance"])), "was_damaged_power_authority": deepcopy(dict(authority))}
        leaves.append(row)
...
def _bind_target_already_acted_power_authority_to_ledger(ledger: Mapping[str, Any], authority: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(authority, Mapping) or authority.get("status") != "resolved" or authority.get("trigger_family") != "target_already_acted" or authority.get("canonical_base_power") != 50 or authority.get("selected_base_power") not in {50, 100}: return _result("rejected", "target_already_acted_power_ledger_authority_invalid", {})
    if ledger.get("status") != "evaluable" or not isinstance(ledger.get("terminal_leaves"), tuple): return deepcopy(dict(ledger))
    rows=[]
    for leaf in ledger["terminal_leaves"]:
        if not isinstance(leaf, Mapping) or not isinstance(leaf.get("provenance"), Mapping): return _result("rejected", "target_already_acted_power_ledger_leaf_invalid", {})
        row=deepcopy(dict(leaf)); row["provenance"]={**deepcopy(dict(row["provenance"])),"target_already_acted_power_authority":deepcopy(dict(authority))}; rows.append(row)
    result=deepcopy(dict(ledger)); result["terminal_leaves"]=tuple(rows); result["component_manifest"]={**deepcopy(dict(result.get("component_manifest",{}))),"target_already_acted_power":{"status":"resolved"}}; return result

def _bind_previous_action_failure_power_authority_to_ledger(ledger: Mapping[str, Any], authority: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(authority, Mapping) or authority.get("status") != "resolved" or authority.get("trigger_family") != "previous_move_failed" or authority.get("canonical_base_power") != 75 or authority.get("selected_base_power") not in {75, 150}: return _result("rejected", "previous_action_failure_power_ledger_authority_invalid", {})
    if ledger.get("status") != "evaluable" or not isinstance(ledger.get("terminal_leaves"), tuple): return deepcopy(dict(ledger))
    rows=[]
    for leaf in ledger["terminal_leaves"]:
        if not isinstance(leaf, Mapping) or not isinstance(leaf.get("provenance"), Mapping): return _result("rejected", "previous_action_failure_power_ledger_leaf_invalid", {})
        row=deepcopy(dict(leaf)); row["provenance"]={**deepcopy(dict(row["provenance"])),"previous_action_failure_power_authority":deepcopy(dict(authority))}; rows.append(row)
    result=deepcopy(dict(ledger)); result["terminal_leaves"]=tuple(rows); result["component_manifest"]={**deepcopy(dict(result.get("component_manifest",{}))),"previous_action_failure_power":{"status":"resolved"}}; return result

def _bind_same_turn_stat_drop_power_authority_to_ledger(ledger: Mapping[str, Any], authority: Mapping[str, Any] | None) -> dict[str, Any]:
    if not isinstance(authority,Mapping) or authority.get("status")!="resolved" or authority.get("trigger_family")!="user_stat_was_lowered_this_turn" or authority.get("selected_base_power") not in {75,150}: return _result("rejected","same_turn_stat_drop_power_ledger_authority_invalid",{})
    if ledger.get("status")!="evaluable" or not isinstance(ledger.get("terminal_leaves"),tuple): return deepcopy(dict(ledger))
    rows=[]
    for leaf in ledger["terminal_leaves"]:
        if not isinstance(leaf,Mapping) or not isinstance(leaf.get("provenance"),Mapping): return _result("rejected","same_turn_stat_drop_power_ledger_leaf_invalid",{})
        row=deepcopy(dict(leaf));row["provenance"]={**deepcopy(dict(row["provenance"])),"same_turn_stat_drop_power_authority":deepcopy(dict(authority))};rows.append(row)
    result=deepcopy(dict(ledger));result["terminal_leaves"]=tuple(rows);result["component_manifest"]={**deepcopy(dict(result.get("component_manifest",{}))),"same_turn_stat_drop_power":{"status":"resolved"}};return result

def _bind_rage_fist_hit_count_power_authority_to_ledger(ledger:Mapping[str,Any],authority:Mapping[str,Any]|None)->dict[str,Any]:
    if not isinstance(authority,Mapping) or authority.get("status")!="resolved" or authority.get("selected_base_power") not in {50,100,150,200,250,300,350}:return _result("rejected","rage_fist_hit_count_ledger_authority_invalid",{})
    if ledger.get("status")!="evaluable" or not isinstance(ledger.get("terminal_leaves"),tuple):return deepcopy(dict(ledger))
    rows=[]
    for leaf in ledger["terminal_leaves"]:
        row=deepcopy(dict(leaf));row["provenance"]={**deepcopy(dict(row["provenance"])),"rage_fist_hit_count_power_authority":deepcopy(dict(authority))};rows.append(row)
    result=deepcopy(dict(ledger));result["terminal_leaves"]=tuple(rows);return result

def _bind_last_respects_faint_power_authority_to_ledger(ledger:Mapping[str,Any],authority:Mapping[str,Any]|None)->dict[str,Any]:
    if not isinstance(authority,Mapping) or authority.get("status")!="resolved" or authority.get("trigger_family")!="allied_faint_history":return _result("rejected","last_respects_faint_power_ledger_authority_invalid",{})
    if ledger.get("status")!="evaluable" or not isinstance(ledger.get("terminal_leaves"),tuple):return deepcopy(dict(ledger))
    rows=[]
    for leaf in ledger["terminal_leaves"]:
        row=deepcopy(dict(leaf));row["provenance"]={**deepcopy(dict(row["provenance"])),"last_respects_faint_power_authority":deepcopy(dict(authority))};rows.append(row)
    result=deepcopy(dict(ledger));result["terminal_leaves"]=tuple(rows);return result
...
def _fling_failure_ledger(*, strategy_d0: Mapping[str, Any], actor: Mapping[str, Any], target: Mapping[str, Any], action: Mapping[str, Any], authority: Mapping[str, Any]) -> dict[str, Any]:
    own = strategy_d0.get("strategy_state", {}).get("active", {}).get(actor.get("side"), {}).get("current_hp")
    foe = strategy_d0.get("strategy_state", {}).get("active", {}).get(target.get("side"), {}).get("current_hp")
    if not _hp(own) or not _hp(foe): return _result("incomplete", "fling_failure_hp_authority_missing", {})
    leaf = {"leaf_id": f"{action['action_id']}:fling_failure:{authority.get('outcome')}", "candidate_id": action["action_id"], "action_type": "attack", "branch_path": ("fling_failure", authority.get("outcome")), "probability": _fd(Fraction(1, 1)), "hit_state": "not_applicable", "critical_state": "not_applicable", "damage_roll": "not_applicable", "consequences": {"damage": 0, "own_final_hp": own, "target_final_hp": foe, "target_ko": foe == 0, "self_fainted": own == 0, "secondary": None, "contact": "not_applicable", "fling_execution": deepcopy(dict(authority))}, "provenance": {"session_id": strategy_d0["session_id"], "source_runtime_fingerprint": strategy_d0["source_runtime_fingerprint"], "source_branch_fingerprint": strategy_d0["strategy_preview_fingerprint"], "decision_owner": deepcopy(dict(strategy_d0["decision_owner"])), "attacker": deepcopy(dict(actor)), "target": deepcopy(dict(target)), "move_id": "fling", "fling_execution_authority": deepcopy(dict(authority))}}
    return {"status": "evaluable", "terminal_leaves": (leaf,), "terminal_probability_mass": _fd(Fraction(1, 1)), "provenance": "deterministic_fling_pre_execution_failure_v1"}

def _apply_fling_item_throw_to_ledger(*, ledger: Mapping[str, Any], authority: Mapping[str, Any]) -> dict[str, Any]:
    if ledger.get("status") != "evaluable": return deepcopy(dict(ledger))
    leaves = ledger.get("terminal_leaves")
    if not isinstance(leaves, tuple): return _result("rejected", "fling_ledger_leaves_invalid", {})
    rows = []
    for leaf in leaves:
        throw = materialize_detached_fling_item_throw(authority=authority, source_leaf=leaf)
...
def _apply_damage_based_recoil_to_normal_ledger(*, runtime_snapshot: Mapping[str, Any], ledger: Mapping[str, Any], attacker: Mapping[str, Any], move_metadata: Mapping[str, Any]) -> dict[str, Any]:
    if ledger.get("status") != "evaluable": return deepcopy(dict(ledger))
    leaves=ledger.get("terminal_leaves")
    if not isinstance(leaves,tuple): return _result("rejected","recoil_normal_ledger_leaves_invalid",{})
    updated=[]
    for leaf in leaves:
        result=apply_detached_damage_based_recoil(runtime_snapshot=runtime_snapshot,attacker=attacker,move_metadata=move_metadata,leaf=leaf)
        if result.get("status")!="resolved": return _result(_status(result),result.get("reason","recoil_consequence_unavailable"),{})
        updated.append(result["leaf"])
    result=deepcopy(dict(ledger)); result["terminal_leaves"]=tuple(updated); result["component_manifest"]={**deepcopy(dict(result.get("component_manifest",{}))),"damage_based_recoil":{"status":"resolved"}}
    return result
