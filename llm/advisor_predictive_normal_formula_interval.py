...
def build_predictive_normal_formula_interval(*, branch_state: Mapping[str, Any], decision_owner: Mapping[str, Any], target_owner: Mapping[str, Any], snapshot_damage_input: Mapping[str, Any], stat_provenance: Mapping[str, Any], trusted_level: int | None, is_critical: bool = False) -> dict[str, Any]:
    ...
    if substitute["state"] in {"unknown","legacy_untracked"}: return _base(fp,decision_owner,target_owner,eligibility["move_id"],"exact_incomplete","substitute_state_unknown",is_critical=is_critical)
    ...
    if native.get("status")!="known": return _base(fp,decision_owner,target_owner,eligibility["move_id"],"unsupported" if native.get("status")=="unsupported_mechanic" else "exact_incomplete",native.get("unsupported_reason") or "native_evaluator_incomplete",native=native,is_critical=is_critical)
    rolls=native.get("exact_damage_rolls")
    if not isinstance(rolls,tuple) or len(rolls)!=16 or any(isinstance(x,bool) or not isinstance(x,int) or x<0 for x in rolls): return _base(fp,decision_owner,target_owner,eligibility["move_id"],"exact_incomplete","native_roll_set_unavailable",native=native,is_critical=is_critical)
