...
def normal_formula_eligibility(move: Mapping[str, Any]) -> dict[str, Any]:
    ...
    guard = _CHARGE_MOVES.immediate_execution_guard(move["move_id"])
    if guard is not None: return {"status":"unsupported","reason":guard["reason"],"execution_guard":guard}
...
def build_predictive_normal_formula_interval(*, branch_state: Mapping[str, Any], decision_owner: Mapping[str, Any], target_owner: Mapping[str, Any], snapshot_damage_input: Mapping[str, Any], stat_provenance: Mapping[str, Any], trusted_level: int | None, is_critical: bool = False, is_spread: bool = False, source_runtime_fingerprint: str | None = None) -> dict[str, Any]:
    ...
    if not isinstance(is_critical,bool) or not isinstance(is_spread,bool): return _r("rejected","invalid_critical_or_spread_damage_scope")
    ...
    if substitute["state"] in {"unknown","legacy_untracked"}: return _base(fp,decision_owner,target_owner,eligibility["move_id"],"exact_incomplete","substitute_state_unknown",is_critical=is_critical,source_runtime_fingerprint=source_runtime_fingerprint)
    native=evaluate_direct_damage_mechanics(deepcopy(dict(snapshot_damage_input)),stat_provenance=deepcopy(dict(stat_provenance)),trusted_level=trusted_level,is_critical=is_critical,is_spread=is_spread)
    if native.get("status")!="known": return _base(fp,decision_owner,target_owner,eligibility["move_id"],"unsupported" if native.get("status")=="unsupported_mechanic" else "exact_incomplete",native.get("unsupported_reason") or "native_evaluator_incomplete",native=native,is_critical=is_critical,source_runtime_fingerprint=source_runtime_fingerprint)
    rolls=native.get("exact_damage_rolls")
    if not isinstance(rolls,tuple) or len(rolls)!=16 or any(isinstance(x,bool) or not isinstance(x,int) or x<0 for x in rolls): return _base(fp,decision_owner,target_owner,eligibility["move_id"],"exact_incomplete","native_roll_set_unavailable",native=native,is_critical=is_critical,source_runtime_fingerprint=source_runtime_fingerprint)
    ...
    return {**_base(fp,decision_owner,target_owner,eligibility["move_id"],"exact_complete","native_q12_exact_roll_set",native=native,is_critical=is_critical,source_runtime_fingerprint=source_runtime_fingerprint),"spread_damage_scope":"spread" if is_spread else "single_target","exact_damage_rolls":rolls,"min_damage":lo,"max_damage":hi,"target_routing":route,"target_hp_before":hp if route=="target" else None,"reachable_post_hit_hp":remaining if route=="target" else None,"post_hit_hp_min":min(remaining) if route=="target" else None,"post_hit_hp_max":max(remaining) if route=="target" else None,"reachable_substitute_hp":remaining if route=="substitute" else None,"guaranteed_facts":facts}

def _base(fp,o,t,move_id,completeness,reason,native=None,is_critical=False,source_runtime_fingerprint=None):
 ...
 if isinstance(source_runtime_fingerprint,str) and source_runtime_fingerprint:r["source_runtime_fingerprint"]=source_runtime_fingerprint
