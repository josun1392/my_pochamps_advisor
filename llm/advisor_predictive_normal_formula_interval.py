...
def normal_formula_eligibility(move: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if any(move.get(key) not in {None,0,False} for key in ("min_hits","max_hits","drain","recoil","charge_turn","recharge","self_ko")): return {"status":"unsupported","reason":"not_simple_normal_formula_move"}
