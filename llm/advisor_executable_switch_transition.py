...
def execute_materialized_switch_entry(
    *, materialized_switch: Mapping[str, Any], entry_authority: Mapping[str, Any],
    ...
    This deliberately consumes no manual candidate or action-order evidence, so
    a forced switch can reuse the same entry authority without becoming a
    voluntary switch.  It remains limited to the entry effects already
    supported by this module.
    """
    if materialized_switch.get("status") != "resolved":
        return _result("rejected", "resolved_incoming_materialization_required")
    state = deepcopy(materialized_switch.get("next_state")) if isinstance(materialized_switch.get("next_state"), Mapping) else None
    source_fp = materialized_switch.get("resulting_branch_fingerprint")
    if not isinstance(state, dict) or fingerprint_transition_preview_state(state) != source_fp:
        return _result("rejected", "stale_or_invalid_materialized_switch_branch")
    target, hazards = entry_authority.get("target_roster_mechanics"), entry_authority.get("hazards")
    if not isinstance(target, Mapping) or not isinstance(hazards, Mapping):
        return _result("incomplete", "switch_entry_authority")
    incoming_side = target.get("side") if isinstance(target, Mapping) else None
    opposing_side = "opponent" if incoming_side == "self" else "self" if incoming_side == "opponent" else None
    ...
    state = projected["next_state"]; active = state["active"][incoming_side]
    damage = entry["damage"]
    active["current_hp"] = max(0, active["current_hp"] - damage); active["fainted"] = active["current_hp"] == 0
    _sync_hp(state, incoming_side, active["current_hp"], active["max_hp"])
    ...
    toxic = entry.get("toxic_spikes_result"); entry_fp = fingerprint_transition_preview_state(state)
    if not isinstance(toxic, Mapping) or toxic.get("status") != "complete": return _result("incomplete", "toxic_spikes_entry_authority")
    if toxic.get("outcome") == "absorbed": changed = remove_absorbed_toxic_spikes(branch_state=state, source_fingerprint=entry_fp, absorption=toxic)
    elif toxic.get("outcome") == "status_applied": changed = apply_toxic_spikes_condition(branch_state=state, branch_fingerprint=entry_fp, owner=state["active"][incoming_side], evaluator_result=toxic)
    elif toxic.get("outcome") in {"absent", "ungrounded", "status_immune", "already_statused", "prevented_by_heavy_duty_boots", "status_prevented"}: changed = {"status": "resolved", "next_state": state, "resulting_branch_fingerprint": entry_fp}
    ...
    return {"status": "resolved", "post_switch_branch_fingerprint": source_fp, "post_entry_branch_fingerprint": entry_fp, "resulting_branch_fingerprint": fingerprint_transition_preview_state(state), "next_state": state, "entry_effect_result": deepcopy(entry), "consequence_trace": trace, "boundary": {"phase": "pre_end_of_turn"}}
...
def execute_manual_switch_then_direct(
    *, source_branch: Mapping[str, Any], source_branch_fingerprint: str, switch_snapshot: Mapping[str, Any],
    ...
    entry = execute_materialized_switch_entry(
        materialized_switch=materialized,
        entry_authority={
            "hazards": hazards, "target_roster_mechanics": target,
            "intimidate_authority": post.get("switch_entry_intimidate_authority"),
            "download_authority": post.get("switch_entry_download_authority"),
            "field_state_context": post.get("side_shared_authority", {}).get("field_state_context") if isinstance(post.get("side_shared_authority"), Mapping) else None,
        },
    )
    if entry.get("status") == "unsupported" and entry.get("reason") == "replacement_required_after_entry_hazard_ko":
        return {**entry, "source_branch_fingerprint": source_branch_fingerprint}
    if entry.get("status") != "resolved": return entry
    state, entry_fp, post_switch_fp, trace = entry["next_state"], entry["post_entry_branch_fingerprint"], entry["post_switch_branch_fingerprint"], entry["consequence_trace"]
    ...
    if direct.get("status") != "resolved":
        return direct
    return {"status": "resolved", "source_snapshot_fingerprint": source_branch_fingerprint, "source_branch_fingerprint": source_branch_fingerprint, "post_switch_branch_fingerprint": post_switch_fp, "post_entry_branch_fingerprint": entry_fp, "resulting_branch_fingerprint": fingerprint_transition_preview_state(direct["next_state"]), "switch_transition": deepcopy(switch), "entry_effect_result": deepcopy(entry["entry_effect_result"]), "direct_evaluation": deepcopy(evaluated), "consequence_trace": trace + deepcopy(direct["consequence_trace"]), "next_state": deepcopy(direct["next_state"]), "boundary": {"phase": "pre_end_of_turn"}, "limitations": ["switch_first_only", "bounded_switch_entry_effects_only", "no_reducer_or_runtime_writeback"]}
...
def _sync_hp(state: Mapping[str, Any], side: str, hp: int, maximum: int) -> None:
    ...
    if isinstance(combatant, dict): combatant["current_hp"], combatant["max_hp"] = hp, maximum


def _sync_speed_stage(state: Mapping[str, Any], side: str, stage: Any) -> bool:
    ...
    match = next((row for row in rows if isinstance(row, dict) and row.get("side") == side and row.get("stat") == "speed" and row.get("status") == "user_confirmed" and row.get("source") == "user_confirmed_current_stat_stage" and row.get("confidence") == "known"), None) if isinstance(rows, list) else None
    if match is None: return False
    match["stage"] = stage
    role = "attacker" if side == "self" else "defender"
    combatant = current.get("direct_mechanics_context", {}).get(role) if isinstance(current, Mapping) and isinstance(current.get("direct_mechanics_context"), Mapping) else None
    if isinstance(combatant, dict) and isinstance(combatant.get("boosts"), dict): combatant["boosts"]["speed"] = stage
    return True


def _materialized_has_ability(state: Mapping[str, Any], side: str, ability: str) -> bool:
    current = state.get("current_state") if isinstance(state, Mapping) else None
    rows = current.get("ability_context", {}).get("current_abilities") if isinstance(current, Mapping) else None
    return any(isinstance(row, Mapping) and row.get("side") == side and row.get("ability") == ability and row.get("status") == "user_confirmed" and row.get("source") == "user_confirmed_current_ability" for row in rows) if isinstance(rows, list) else False


def _sync_attack_stage(state: Mapping[str, Any], side: str | None, target: Any, before: Any, after: Any) -> bool:
    opposing = state.get("active", {}).get(side) if isinstance(state.get("active"), Mapping) else None
    if side not in {"self", "opponent"} or not isinstance(opposing, Mapping) or not isinstance(target, Mapping) or any(opposing.get(key) != target.get(key) for key in ("side", "slot_index", "pokemon_id")):
        return False
    if any(isinstance(value, bool) or not isinstance(value, int) or not -6 <= value <= 6 for value in (before, after)):
        return False
    current = state.get("current_state") if isinstance(state, Mapping) else None
    rows = current.get("stat_stage_context", {}).get("current_stages") if isinstance(current, Mapping) else None
    match = next((row for row in rows if isinstance(row, dict) and row.get("side") == side and row.get("stat") == "attack" and row.get("status") == "user_confirmed" and row.get("source") == "user_confirmed_current_stat_stage" and row.get("confidence") == "known"), None) if isinstance(rows, list) else None
...
def _sync_offensive_stage(state: Mapping[str, Any], side: str, stat: Any, before: Any, after: Any) -> bool:
    ...
    current = state.get("current_state") if isinstance(state, Mapping) else None
    rows = current.get("stat_stage_context", {}).get("current_stages") if isinstance(current, Mapping) else None
    match = next((row for row in rows if isinstance(row, dict) and row.get("side") == side and row.get("stat") == stat and row.get("status") == "user_confirmed" and row.get("source") == "user_confirmed_current_stat_stage" and row.get("confidence") == "known"), None) if isinstance(rows, list) else None
    if match is None or match.get("stage") != before:
        return False
    match["stage"] = after
    return True
