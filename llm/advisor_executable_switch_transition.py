...
def execute_materialized_switch_entry(
    *, materialized_switch: Mapping[str, Any], entry_authority: Mapping[str, Any],
    ...
    active = state.get("active", {}).get("self") if isinstance(state.get("active"), Mapping) else None
    ...
    state = projected["next_state"]; active = state["active"]["self"]
    damage = entry["damage"]
    ...
    if intimidate.get("outcome") in {"attack_stage_lowered", "attack_stage_minimum"}:
        if not _materialized_self_has_ability(state, "intimidate") or not _sync_opponent_attack_stage(state, intimidate.get("opponent_identity"), intimidate.get("attack_stage_before"), intimidate.get("attack_stage_after")): return _result("incomplete", "opponent_exact_attack_stage_authority")
    ...
    if download.get("outcome") in {"attack_stage_raised", "attack_stage_maximum", "special-attack_stage_raised", "special-attack_stage_maximum"}:
        if not _materialized_self_has_ability(state, "download") or not _sync_self_offensive_stage(state, download.get("boosted_stat"), download.get("stage_before"), download.get("stage_after")): return _result("incomplete", "incoming_exact_download_stage_authority")
    ...
    if weather.get("outcome") in {"weather_set", "weather_already_active"}:
        ...
        if ability is None or not _materialized_self_has_ability(state, ability): return _result("unsupported", "weather_entry_outcome")
    ...
    if sticky.get("outcome") in {"speed_stage_lowered", "speed_stage_minimum"} and not _sync_self_speed_stage(state, sticky.get("speed_stage_after")): return _result("incomplete", "incoming_exact_speed_stage_authority")
    ...
    elif toxic.get("outcome") == "status_applied": changed = apply_toxic_spikes_condition(branch_state=state, branch_fingerprint=entry_fp, owner=state["active"]["self"], evaluator_result=toxic)
