...
def build_fixed_damage_assessment(selected_move: Mapping[str, Any] | None, current_hp_context: Mapping[str, Any] | None, pokemon: Mapping[str, Any] | None = None, attacker_level_context: Mapping[str, Any] | None = None) -> dict[str, Any] | None:
    """Calculate only explicit fixed-damage rules; never fall back to normal damage."""
    if not isinstance(selected_move, Mapping) or not isinstance(selected_move.get("move_id"), str):
        return None
    move = selected_move["move_id"].lower()
    if move not in _SUPPORTED_FIXED_DAMAGE_RULES and move not in _UNSUPPORTED_FIXED_DAMAGE_MOVES:
        return None
    base = {"move": move, "scope": FIXED_DAMAGE_SCOPE}
    if move in _UNSUPPORTED_FIXED_DAMAGE_MOVES:
        return {**base, "status": "unavailable", "reason": "unsupported_fixed_damage_rule"}
    rule = _SUPPORTED_FIXED_DAMAGE_RULES[move]
    if rule == "attacker-level":
        level = attacker_level_context.get("level") if isinstance(attacker_level_context, Mapping) else None
        if level is None:
            return {**base, "status": "unavailable", "reason": "missing_attacker_level"}
        if isinstance(level, bool) or not isinstance(level, int) or not 1 <= level <= 100:
            return {**base, "status": "unavailable", "reason": "invalid_attacker_level"}
        damage = level
    elif rule.startswith("literal"):
        damage = int(rule.rsplit("-", 1)[1])
    else:
        hp = next((entry for entry in (current_hp_context or {}).get("current_hp", []) if isinstance(entry, Mapping) and entry.get("side") == "opponent"), None) if isinstance(current_hp_context, Mapping) else None
        if not isinstance(hp, Mapping) or "current_hp" not in hp:
            return {**base, "status": "unavailable", "reason": "missing_defender_current_hp"}
        current = hp.get("current_hp")
        if isinstance(current, bool) or not isinstance(current, int) or current < 0:
            return {**base, "status": "unavailable", "reason": "invalid_defender_current_hp"}
        if current == 0:
            return {**base, "status": "not_applicable", "reason": "target_already_fainted"}
        damage = current // 2
    move_type = {"seismic-toss": "normal", "night-shade": "ghost"}.get(move)
    defender_types = _selected_types(pokemon, "opponent_active") if move_type else None
    if move_type and defender_types is not None:
        if any(load_type_chart()[move_type][target] == 0 for target in defender_types):
            return {**base, "rule": rule, "damage": 0, "ko_status": "no_ko", "status": "resolved", "reason": "type_immunity"}
    hp = next((entry for entry in (current_hp_context or {}).get("current_hp", []) if isinstance(entry, Mapping) and entry.get("side") == "opponent"), None) if isinstance(current_hp_context, Mapping) else None
    result: dict[str, Any] = {**base, "rule": rule, "damage": damage, "status": "resolved"}
    if rule == "attacker-level": result["attacker_level"] = damage
    if isinstance(hp, Mapping) and isinstance(hp.get("current_hp"), int) and not isinstance(hp.get("current_hp"), bool):
        current = hp["current_hp"]; result["ko_status"] = "guaranteed_ko" if damage >= current else "no_ko"
        if rule == "defender-current-hp-half": result.update(defender_current_hp=current, resulting_hp=current-damage)
        maximum = hp.get("maximum_hp")
        if isinstance(maximum, int) and not isinstance(maximum, bool) and maximum > 0: result["damage_percent"] = round(damage * 100 / maximum, 1)
    return result


def build_hp_based_special_damage_assessment(selected_move: Mapping[str, Any] | None, current_hp_context: Mapping[str, Any] | None, pokemon: Mapping[str, Any] | None = None) -> dict[str, Any] | None:
    if not isinstance(selected_move, Mapping) or selected_move.get("move_id") not in {"endeavor", "final-gambit"}:
        return None
    move = selected_move["move_id"]
    entries = (current_hp_context or {}).get("current_hp", []) if isinstance(current_hp_context, Mapping) else []
    by_side = {entry.get("side"): entry for entry in entries if isinstance(entry, Mapping)}
    base = {"move": move, "scope": HP_BASED_SPECIAL_DAMAGE_SCOPE}
    def hp(side: str) -> tuple[int | None, str | None]:
        entry = by_side.get(side)
        if not isinstance(entry, Mapping) or "current_hp" not in entry: return None, f"missing_{side}_current_hp"
        value, maximum = entry.get("current_hp"), entry.get("maximum_hp")
        if isinstance(value, bool) or not isinstance(value, int) or value < 0 or (maximum is not None and (isinstance(maximum, bool) or not isinstance(maximum, int) or maximum < 1 or value > maximum)):
            return None, f"invalid_{side}_current_hp"
        return value, None
    self_hp, error = hp("self")
    if error: return {**base, "status": "unavailable", "reason": error}
    opponent_hp, error = hp("opponent")
    if error: return {**base, "status": "unavailable", "reason": error}
    assert self_hp is not None and opponent_hp is not None
    if self_hp == 0: return {**base, "status": "not_applicable", "reason": "user_already_fainted"}
    if opponent_hp == 0: return {**base, "status": "not_applicable", "reason": "target_already_fainted"}
    move_type = "normal" if move == "endeavor" else "fighting"
    defender_types = _selected_types(pokemon, "opponent_active")
    immune = defender_types is not None and any(load_type_chart()[move_type][target] == 0 for target in defender_types)
    if move == "endeavor":
        if immune: return {**base, "rule": "equalize-target-current-hp", "self_current_hp": self_hp, "opponent_current_hp": opponent_hp, "damage": 0, "opponent_resulting_hp": opponent_hp, "ko_status": "no_ko", "status": "no_effect", "reason": "type_immunity"}
        if opponent_hp <= self_hp: return {**base, "rule": "equalize-target-current-hp", "self_current_hp": self_hp, "opponent_current_hp": opponent_hp, "damage": 0, "opponent_resulting_hp": opponent_hp, "ko_status": "no_ko", "status": "no_effect", "reason": "target_hp_not_higher"}
        return {**base, "rule": "equalize-target-current-hp", "self_current_hp": self_hp, "opponent_current_hp": opponent_hp, "damage": opponent_hp-self_hp, "opponent_resulting_hp": self_hp, "ko_status": "no_ko", "status": "resolved"}
    actual = 0 if immune else min(self_hp, opponent_hp)
    remaining = opponent_hp - actual
...
def build_target_hp_based_power_assessment(selected_move: Mapping[str, Any] | None, current_hp_context: Mapping[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(selected_move, Mapping) or selected_move.get("move_id") not in _TARGET_HP_POWER_MOVES: return None
    move=selected_move["move_id"]; base={"move":move,"scope":TARGET_HP_POWER_SCOPE}
    entries=(current_hp_context or {}).get("current_hp",[]) if isinstance(current_hp_context,Mapping) else []
    entry=next((item for item in entries if isinstance(item,Mapping) and item.get("side")=="opponent"),None)
