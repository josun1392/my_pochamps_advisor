...
def project_ice_body_end_of_turn(*, pre_end_of_turn: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not _snow_field_authority(state, source_fp, owners["self"]["session_id"]):
        return _result("rejected", "stale_or_invalid_branch_snow_authority")
    if _requires_residual_ordering(state):
        return _result("incomplete", "ice_body_residual_ordering_unresolved")
    abilities = {side: _ability(state, side) for side in ("self", "opponent")}
    if any(value is None for value in abilities.values()):
        return _result("incomplete", "ice_body_current_ability_authority")
    trace: list[dict[str, Any]] = []
    for side in ("self", "opponent"):
        active = state["active"][side]
        if abilities[side] != "ice-body":
            continue
        if active["fainted"]:
            trace.append({"sequence": len(trace) + 1, "effect": "ice_body_recovery", "owner": deepcopy(owners[side]), "weather": "snow", "execution_status": "skipped", "provenance": "detached_branch_ice_body_recovery_v1", "status": "complete", "outcome": "fainted_before_eot"})
            continue
        recovery = evaluate_ice_body_recovery(active_abilities=abilities, target_side=side, current_hp=active["current_hp"], maximum_hp=active["max_hp"])
        if recovery.get("status") != "complete":
            return _result("incomplete", "canonical_ice_body_authority")
        if "post_hp" in recovery:
            active["current_hp"] = recovery["post_hp"]
            _sync_hp(state, side, recovery["post_hp"], recovery["max_hp"])
        trace.append({"sequence": len(trace) + 1, "effect": "ice_body_recovery", "owner": deepcopy(owners[side]), "weather": "snow", "execution_status": "prevented" if recovery.get("outcome", "").startswith("suppressed") else "executed", "provenance": "detached_branch_ice_body_recovery_v1", **deepcopy(recovery)})
    if not trace:
        return _result("incomplete", "ice_body_active_ability_required")
    return {"status": "resolved", "source_pre_end_of_turn_fingerprint": source_fp, "resulting_branch_fingerprint": fingerprint_transition_preview_state(state), "eot_consequence_trace": trace, "next_state": state, "boundary": {"phase": "end_of_turn"}, "limitations": ["ice_body_snow_only", "poison_toxic_or_sandstorm_ordering_fails_closed", "no_reducer_or_runtime_writeback"]}
...
def _snow_field_authority(state: Mapping[str, Any], source_fp: str, session: str) -> bool:
