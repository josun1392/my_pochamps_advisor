...
CURRENT_HP_SOURCE = "ui_current_hp_confirmation"
...
PENDING_CONFUSION_ACTION_EXECUTION_SOURCE = "ui_pending_confusion_action_execution_confirmation"
...
_KINDS["current_hp_observed"] = "production_ready"
...
_KINDS["pending_confusion_action_execution_observed"] = "production_ready"
...
def _valid_payload(kind, payload):
    if kind == "current_hp_observed":
        current, maximum = payload.get("current_hp"), payload.get("maximum_hp")
        return (set(payload) == {"current_hp", "maximum_hp"}
                and type(current) is int and type(maximum) is int
                and maximum >= 1 and 0 <= current <= maximum)
    ...
    if kind == "confusion_self_hit_damage_observed":
        return (set(payload) == {"decision_point","action_id","move_id","confusion_origin_id","source_confusion_observation_id","hp_before","hp_after","self_fainted","disguise_outcome"}
                and all(isinstance(payload.get(key),str) and bool(payload[key]) for key in ("decision_point","action_id","move_id","confusion_origin_id","source_confusion_observation_id"))
                and all(isinstance(payload.get(key),int) and not isinstance(payload.get(key),bool) and payload[key]>=0 for key in ("hp_before","hp_after"))
                and payload["hp_after"]<=payload["hp_before"] and isinstance(payload.get("self_fainted"),bool)
    ...
    if kind == "flinch_causality_observed":
        scalar_keys = {"producer_source_action_id", "producer_move_id", "producer_owner", "producer_execution_observation_id", "affected_owner", "cancelled_source_action_id", "cancelled_move_id", "cancelled_execution_observation_id", "cancelled_result_observation_id", "producer_predictive_ledger_fingerprint"}
        charge_keys = scalar_keys | {"producer_prediction_kind", "producer_prediction_fingerprint", "original_charge_action_id", "terminal_action_identity", "execution_mode", "caller_kind", "charge_lifecycle"}
        keys = set(payload)
        if frozenset(keys) not in {frozenset(scalar_keys), frozenset(charge_keys)} or payload.get("producer_source_action_id") == payload.get("cancelled_source_action_id"): return False
        strings = ["producer_source_action_id", "producer_move_id", "producer_execution_observation_id", "cancelled_source_action_id", "cancelled_move_id", "cancelled_execution_observation_id", "cancelled_result_observation_id", "producer_predictive_ledger_fingerprint"]
        if keys == charge_keys:
            strings += ["producer_prediction_kind", "producer_prediction_fingerprint", "original_charge_action_id", "terminal_action_identity", "execution_mode", "caller_kind"]
            if payload.get("producer_prediction_kind") != "standard_charge_terminal_execution" or not isinstance(payload.get("charge_lifecycle"), dict): return False
        if not all(isinstance(payload.get(key), str) and bool(payload[key]) for key in strings): return False
...
def _valid_owner_payload(value):
    return (isinstance(value, dict) and set(value) == {"session_id","side","slot_index","pokemon_id"}
