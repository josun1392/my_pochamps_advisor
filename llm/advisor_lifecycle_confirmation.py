...
def _valid_payload(kind, payload):
    ...
    if kind == "exact_hp_recovery_observed": return all(isinstance(payload.get(key), int) and not isinstance(payload.get(key), bool) and payload[key] >= 0 for key in ("hp_before", "hp_after")) and payload["hp_after"] >= payload["hp_before"]
