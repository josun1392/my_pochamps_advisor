...
_KINDS = {"direct_move_damage_observed": "production_ready", "used_move_observed": "production_ready", "exact_hp_transition_observed": "production_ready", "pokemon_switch_observed": "production_ready", "pokemon_faint_observed": "production_ready", "condition_applied_observed": "production_ready", "stat_stage_observed": "production_ready", "switch_hazards_observed": "production_ready", "condition_removed_observed": "fixture_only", "item_consumption_observed": "fixture_only", "item_removed_observed": "fixture_only", "weather_started_observed": "fixture_only", "weather_ended_observed": "fixture_only", "terrain_started_observed": "fixture_only", "terrain_ended_observed": "fixture_only", "side_condition_started_observed": "fixture_only", "side_condition_ended_observed": "fixture_only"}
...
class LifecycleConfirmationBoundary:
    ...
    def confirm(self, *, event_kind, payload, session_id, source, trust, confirmed, side=None, slot_index=None, pokemon_id=None, observation_id=None, related_observation_id=None, turn_number=None, production=True):
        ...
        if event_kind not in {"direct_move_damage_observed", "switch_hazards_observed"} and not _owner_matches(self._owners, side, slot_index, pokemon_id): return _result("invalid_provenance", "owner_mismatch", readiness)
        if event_kind == "switch_hazards_observed" and side not in {"self", "opponent"}: return _result("invalid_provenance", "side_owner_mismatch", readiness)
        ...
        if event_kind == "exact_hp_transition_observed": record.update(hp_before=payload["hp_before"], hp_after=payload["hp_after"], hp_unit="exact")
