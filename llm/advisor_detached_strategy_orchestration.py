...
from llm.advisor_predictive_probabilistic_self_stage_effect_uncertainty import compose_predictive_probabilistic_self_stage_effect_uncertainty
 ...
 for candidate in enriched["candidates"]:
  ...
  if candidate["action_type"]=="attack" and isinstance(normal_input,Mapping) and all(isinstance(normal_input.get(key),Mapping) for key in ("target_owner","snapshot_damage_input","stat_provenance")):
   ...
   if interval.get("completeness")=="exact_complete":
    ...
    self_stage_authority=self_stage_probabilities.get(cid)
    target_stage_authority=target_stage_probabilities.get(cid)
    thunderbolt_paralysis_authority=thunderbolt_status_probabilities.get(cid)
    if cid=="attack:metal-claw" and not isinstance(self_stage_authority,Mapping): evidence.append(_e(candidate,"incomplete",reason="probabilistic_self_stage_effect_authority_missing"));continue
    if cid=="attack:shadow-ball" and not isinstance(target_stage_authority,Mapping): evidence.append(_e(candidate,"incomplete",reason="probabilistic_target_stage_effect_authority_missing"));continue
    if cid=="attack:thunderbolt" and not isinstance(thunderbolt_paralysis_authority,Mapping): evidence.append(_e(candidate,"incomplete",reason="thunderbolt_paralysis_authority_missing"));continue
    if isinstance(self_stage_authority,Mapping) and self_stage_authority.get("status")!="resolved": evidence.append(_e(candidate,"incomplete",reason=self_stage_authority.get("reason","probabilistic_self_stage_effect_authority_unavailable"),probabilistic_self_stage_effect_authority=self_stage_authority));continue
    if isinstance(target_stage_authority,Mapping) and target_stage_authority.get("status")!="resolved": evidence.append(_e(candidate,"incomplete",reason=target_stage_authority.get("reason","probabilistic_target_stage_effect_authority_unavailable"),probabilistic_target_stage_effect_authority=target_stage_authority));continue
    if isinstance(thunderbolt_paralysis_authority,Mapping) and thunderbolt_paralysis_authority.get("status")!="resolved": evidence.append(_e(candidate,"incomplete",reason=thunderbolt_paralysis_authority.get("reason","thunderbolt_paralysis_authority_unavailable"),thunderbolt_paralysis_authority=thunderbolt_paralysis_authority));continue
    sturdy_survival_authority=sturdy_survivals.get(cid); focus_sash_survival_authority=focus_sash_survivals.get(cid)
    fact=_normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=cid=="attack:water-gun" and legacy_water,probabilistic_self_stage_effect_authority=self_stage_authority,probabilistic_target_stage_effect_authority=target_stage_authority,thunderbolt_paralysis_authority=thunderbolt_paralysis_authority,sturdy_survival_authority=sturdy_survival_authority,focus_sash_survival_authority=focus_sash_survival_authority)
    if isinstance(fact.get("post_hit_failure"),Mapping): evidence.append(_e(candidate,"incomplete",reason=fact["post_hit_failure"].get("reason","target_survival_authority_unavailable"),post_hit_failure=fact["post_hit_failure"]));continue
    if isinstance(self_stage_authority,Mapping) and not isinstance(fact.get("probabilistic_self_stage_effect_uncertainty"),Mapping): evidence.append(_e(candidate,"incomplete",reason="probabilistic_self_stage_effect_uncertainty_unavailable"));continue
    if isinstance(target_stage_authority,Mapping) and not isinstance(fact.get("probabilistic_target_stage_effect_uncertainty"),Mapping): evidence.append(_e(candidate,"incomplete",reason="probabilistic_target_stage_effect_uncertainty_unavailable"));continue
    if isinstance(thunderbolt_paralysis_authority,Mapping) and not isinstance(fact.get("thunderbolt_paralysis_uncertainty"),Mapping): evidence.append(_e(candidate,"incomplete",reason="thunderbolt_paralysis_uncertainty_unavailable"));continue
    hit_consequences={"interval":interval,"post_hit":fact.get("post_hit"),"stage_effects":fact.get("stage_effects"),"damage_roll_uncertainty":fact.get("damage_roll_uncertainty"),"probabilistic_self_stage_effect_uncertainty":fact.get("probabilistic_self_stage_effect_uncertainty"),"probabilistic_target_stage_effect_uncertainty":fact.get("probabilistic_target_stage_effect_uncertainty"),"thunderbolt_paralysis_uncertainty":fact.get("thunderbolt_paralysis_uncertainty"),"guaranteed_facts":fact}
    ...
    if isinstance(critical_probability,Mapping):
     ...
     critical_fact=_normal_formula_facts(candidate,critical_interval,own,post_input,normal_input,probabilistic_self_stage_effect_authority=self_stage_authority,probabilistic_target_stage_effect_authority=target_stage_authority,thunderbolt_paralysis_authority=thunderbolt_paralysis_authority,sturdy_survival_authority=sturdy_survival_authority,focus_sash_survival_authority=focus_sash_survival_authority)
     if isinstance(critical_fact.get("post_hit_failure"),Mapping): evidence.append(_e(candidate,"incomplete",reason=critical_fact["post_hit_failure"].get("reason","target_survival_authority_unavailable"),post_hit_failure=critical_fact["post_hit_failure"]));continue
     critical_uncertainty=compose_predictive_critical_hit_uncertainty(candidate=candidate,strict_critical_hit_probability=critical_probability,paired_damage_contexts=paired,non_critical_consequences=hit_consequences,critical_consequences={"interval":critical_interval,"post_hit":critical_fact.get("post_hit"),"stage_effects":critical_fact.get("stage_effects"),"damage_roll_uncertainty":critical_fact.get("damage_roll_uncertainty"),"probabilistic_self_stage_effect_uncertainty":critical_fact.get("probabilistic_self_stage_effect_uncertainty"),"probabilistic_target_stage_effect_uncertainty":critical_fact.get("probabilistic_target_stage_effect_uncertainty"),"thunderbolt_paralysis_uncertainty":critical_fact.get("thunderbolt_paralysis_uncertainty"),"guaranteed_facts":critical_fact})
...
def _rank(facts,evidence,ledgers,metrics,response_profiles=None):
...
def _normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=False,probabilistic_self_stage_effect_authority=None,probabilistic_target_stage_effect_authority=None,thunderbolt_paralysis_authority=None,iron_head_flinch_authority=None,sparkling_aria_burn_clearing_authority=None,sturdy_survival_authority=None,focus_sash_survival_authority=None,life_orb_authority_context=None,endure_turn_context=None):
 ...
 if isinstance(post_input,Mapping):
  ...
  elif any(isinstance(authority,Mapping) for authority in (sturdy_survival_authority,focus_sash_survival_authority,endure_turn_context)): fact={**fact,"post_hit_failure":composed}
 ...
 if isinstance(probabilistic_self_stage_effect_authority,Mapping):
