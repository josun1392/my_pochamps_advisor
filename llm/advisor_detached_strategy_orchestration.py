 ...
 for candidate in enriched["candidates"]:
  ...
  if candidate["action_type"]=="attack" and isinstance(normal_input,Mapping) and all(isinstance(normal_input.get(key),Mapping) for key in ("target_owner","snapshot_damage_input","stat_provenance")):
   ...
   if interval.get("completeness")=="exact_complete":
    ...
    fact=_normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=cid=="attack:water-gun" and legacy_water,probabilistic_self_stage_effect_authority=self_stage_authority,probabilistic_target_stage_effect_authority=target_stage_authority)
    ...
    hit_consequences={"interval":interval,"post_hit":fact.get("post_hit"),"stage_effects":fact.get("stage_effects"),"damage_roll_uncertainty":fact.get("damage_roll_uncertainty"),"probabilistic_self_stage_effect_uncertainty":fact.get("probabilistic_self_stage_effect_uncertainty"),"probabilistic_target_stage_effect_uncertainty":fact.get("probabilistic_target_stage_effect_uncertainty"),"guaranteed_facts":fact}
    ...
    if isinstance(critical_probability,Mapping):
     ...
     critical_fact=_normal_formula_facts(candidate,critical_interval,own,post_input,normal_input,probabilistic_self_stage_effect_authority=self_stage_authority,probabilistic_target_stage_effect_authority=target_stage_authority)
     critical_uncertainty=compose_predictive_critical_hit_uncertainty(candidate=candidate,strict_critical_hit_probability=critical_probability,paired_damage_contexts=paired,non_critical_consequences=hit_consequences,critical_consequences={"interval":critical_interval,"post_hit":critical_fact.get("post_hit"),"stage_effects":critical_fact.get("stage_effects"),"damage_roll_uncertainty":critical_fact.get("damage_roll_uncertainty"),"probabilistic_self_stage_effect_uncertainty":critical_fact.get("probabilistic_self_stage_effect_uncertainty"),"probabilistic_target_stage_effect_uncertainty":critical_fact.get("probabilistic_target_stage_effect_uncertainty"),"guaranteed_facts":critical_fact})
...
def _normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=False,probabilistic_self_stage_effect_authority=None,probabilistic_target_stage_effect_authority=None):
