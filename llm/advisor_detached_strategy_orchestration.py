 ...
 for candidate in enriched["candidates"]:
  ...
  if candidate["action_type"]=="attack" and isinstance(normal_input,Mapping) and all(isinstance(normal_input.get(key),Mapping) for key in ("target_owner","snapshot_damage_input","stat_provenance")):
   ...
   if interval.get("completeness")=="exact_complete":
    ...
    fact=_normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=cid=="attack:water-gun" and legacy_water,probabilistic_self_stage_effect_authority=self_stage_authority)
    ...
    hit_consequences={"interval":interval,"post_hit":fact.get("post_hit"),"stage_effects":fact.get("stage_effects"),"damage_roll_uncertainty":fact.get("damage_roll_uncertainty"),"probabilistic_self_stage_effect_uncertainty":fact.get("probabilistic_self_stage_effect_uncertainty"),"guaranteed_facts":fact}
...
def _normal_formula_facts(candidate,interval,own,post_input,normal_input,legacy_water=False,probabilistic_self_stage_effect_authority=None):
