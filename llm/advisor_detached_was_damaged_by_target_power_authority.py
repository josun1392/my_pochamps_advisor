...
def materialize_detached_was_damaged_by_target_power_authority(*,move:Mapping[str,Any],user:Mapping[str,Any],target:Mapping[str,Any],incoming_event:Mapping[str,Any]|None,source_terminal_leaf:Mapping[str,Any]|None=None)->dict[str,Any]:
 ...
 hits=source_terminal_leaf.get("ordered_hits") if isinstance(source_terminal_leaf,Mapping) else None
 if isinstance(hits,(tuple,list)) and isinstance(incoming_event,Mapping) and incoming_event.get("status")=="resolved" and incoming_event.get("recipient")==user and incoming_event.get("source_attacker")==target and incoming_event.get("qualifying_event") is True:
  ok=any(isinstance(h,Mapping) and h.get("target_routing","target")=="target" and isinstance(h.get("actual_damage"),int) and h["actual_damage"]>0 for h in hits)
