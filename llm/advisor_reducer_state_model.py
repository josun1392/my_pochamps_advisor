...
def validate_battle_state_unknown_markers(state):
    ...
    if isinstance(trick_room, str) and trick_room in {"active", "inactive"} and not isinstance(trick_room_provenance, dict):
        return False
    magic_room = field.get("magic_room_status", make_unknown_battle_fact())
    magic_room_provenance = field.get("magic_room_status_provenance")
    if not (is_unknown_battle_fact(magic_room) or (isinstance(magic_room, str) and magic_room in {"active", "inactive"})):
        return False
...
def _valid_current_healing_prevented_state(value, provenance):
    ...
    return (
        ...
        and not isinstance(provenance["turn_number"], bool)
        and provenance["turn_number"] >= 1
...
def _apply(state, event):
    ...
    if effect == "record_champions_confusion_progression":
        from llm.advisor_champions_confusion_progression import observe_confusion_progression
        pokemon = _pokemon(state, event)
        if pokemon is None or not _active_identity_matches(state, _value(event, "side"), _value(event, "slot_index"), _value(event, "pokemon_id")) or pokemon.get("fainted") is True:
            return _conflict(event, "invalid_champions_confusion_progression_owner")
    ...
    if effect == "set_current_hp_snapshot":
        pokemon = _pokemon(state, event)
        current, maximum = _value(event, "current_hp"), _value(event, "maximum_hp")
        if (pokemon is None or not _active_identity_matches(state, _value(event, "side"),
                _value(event, "slot_index"), _value(event, "pokemon_id"))
                or not _exact(current) or not _exact(maximum)
                or maximum < 1 or current > maximum):
            return _conflict(event, "invalid_current_hp_snapshot")
        pokemon["current_hp"] = current
        pokemon["max_hp"] = maximum
        _mark(pokemon, "current_hp", event)
        _mark(pokemon, "max_hp", event)
        return None
...
def _set_current_healing_prevented(state, event):
    pokemon = _pokemon(state, event)
    status, turn_number = _value(event, "status"), _value(event, "turn_number")
    if pokemon is None or not _active_identity_matches(state, _value(event, "side"), _value(event, "slot_index"), _value(event, "pokemon_id")):
        return _conflict(event, "invalid_current_healing_prevented_owner")
    if status not in {"active", "inactive"} or not isinstance(turn_number, int) or isinstance(turn_number, bool) or turn_number < 1:
        return _conflict(event, "invalid_current_healing_prevented")
    prior = pokemon.get("healing_prevented_status_provenance")
    prior_turn = prior.get("turn_number") if isinstance(prior, dict) and prior.get("event_kind") == "current_healing_prevented_observed" else None
    if isinstance(prior_turn, int) and not isinstance(prior_turn, bool) and turn_number < prior_turn:
        return _conflict(event, "stale_current_healing_prevented_observation")
    pokemon["healing_prevented_status"] = status
    pokemon["healing_prevented_status_provenance"] = _provenance(event) | {
        "event_kind": "current_healing_prevented_observed", "trust": _value(event, "trust"),
...
def _observed_magic_room(state, event):
    field = state.get("field"); status = _value(event, "magic_room_status")
    if not isinstance(field, dict) or status not in {"active", "inactive"}:
        return _conflict(event, "invalid_observed_magic_room")
    current = field.get("magic_room_status", "unknown")
    if not (isinstance(current, str) and current in {"active", "inactive", "unknown"}) and not is_unknown_battle_fact(current):
...
def _switch(state, event):
    ...
    if isinstance(outgoing, dict):
        ...
        if outgoing.get("current_confusion") == "confused":
            ...
            outgoing["champions_confusion_progression"] = None
            outgoing["confusion_provenance"] = {"event_kind": "confusion_cleared_on_switch", "trust": "user_confirmed_observation", "turn_number": _value(event, "turn_number"), "state": "none"}
