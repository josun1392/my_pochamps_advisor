...
def freeze_runtime_d0_fling_item_execution_authority(
    *, strategy_d0: Mapping[str, Any], runtime_snapshot: Mapping[str, Any], action: Mapping[str, Any], actor: Mapping[str, Any], target: Mapping[str, Any],
    ...
    type_resist_berry = (
        resolve_canonical_fling_type_resist_empty_intrinsic_berry(item["value"])
        if effect.get("kind") == "berry_effect"
        else {"status": "not_applicable"}
    )
    if type_resist_berry.get("status") == "rejected":
        common["fling_type_resist_empty_intrinsic_berry_authority"] = type_resist_berry
        return _terminal(
            "incomplete_authority",
            "rejected",
            type_resist_berry.get(
                "reason", "fling_type_resist_empty_intrinsic_berry_authority_rejected"
            ),
            base,
            common,
        )
    ...
    lum_berry = (
        resolve_canonical_fling_lum_major_status_confusion_cure_berry(item["value"])
        if effect.get("kind") == "berry_effect"
        else {"status": "not_applicable"}
    )
    if lum_berry.get("status") == "rejected":
        common["fling_lum_major_status_confusion_cure_berry_authority"] = lum_berry
        return _terminal(
            "incomplete_authority",
            "rejected",
            lum_berry.get("reason", "fling_lum_major_status_confusion_cure_berry_authority_rejected"),
            base,
            common,
        )
    supported_lum_berry = lum_berry.get("status") == "resolved"
    hp_restore_berry = (
        resolve_canonical_fling_hp_restore_berry(item["value"])
        if effect.get("kind") == "berry_effect"
        else {"status": "not_applicable"}
    )
    if hp_restore_berry.get("status") == "rejected":
        common["fling_hp_restore_berry_authority"] = hp_restore_berry
        return _terminal(
            "incomplete_authority",
            "rejected",
            hp_restore_berry.get("reason", "fling_hp_restore_berry_authority_rejected"),
            base,
            common,
        )
    ...
    if supported_persim_berry:
        ...
        common["fling_persim_confusion_cure_berry_authority"] = persim_berry
    if supported_lum_berry:
        common["fling_lum_major_status_confusion_cure_support"] = "fling_lum_major_status_confusion_cure_target_effect_v1"
        common["fling_lum_major_status_confusion_cure_berry_authority"] = lum_berry
