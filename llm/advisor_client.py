...
def _build_ui_selected_prompt(battle_input: dict[str, Any]) -> str:
    return (
        ...
        "recovery_context; it does not change raw damage_range, rolls, or "
        "ko_context. recovery_context applies only when Sitrus Berry or "
        ...
        "sequencing is not modeled. Say recovery may affect follow-up KO/2HKO "
        "under limited assumptions; do not claim final 2HKO or 3HKO truth "
        "without Turn Engine, and do not infer recovery if the item is unknown "
        "or unconfirmed. Do not combine Focus Sash and recovery into final "
        "outcome claims. Focus Sash survival may appear only as limited "
