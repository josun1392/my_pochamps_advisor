...
from llm.advisor_candidate_contract import (
    DIRECT_MECHANICS_PROVIDER_CLAIM_KINDS,
...
_STRUCTURED_CLAIM_SCHEMA = {"type": "OBJECT", "properties": {"kind": {"type": "STRING", "enum": ["damage", "ko", "hit_chance", "move_order", "self_effect", "dynamic_mechanic", "partial_context", "mechanics"]}, "claim": {"type": "STRING", "description": "Use only deterministic evidence available for this candidate; do not invent numeric mechanics values or native evidence links."}}, "required": ["kind", "claim"]}
...
def _claim_schema_for_provider_payload(*, provider_payload: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    if statuses and all(status == "known" for status in statuses):
        ...
        schema["properties"] = {
            "kind": {"type": "STRING", "enum": sorted(DIRECT_MECHANICS_PROVIDER_CLAIM_KINDS - {"partial_context"})},
            "claim": {"type": "STRING", "description": "For a numeric claim kind, include only exact native values. value_free_mechanics must contain no digit."},
        }
        schema["required"] = ["kind", "claim"]
...
def _format_validated_selected_candidate_summary(selected: Any) -> list[str]:
    ...
    if isinstance(action_order, Mapping):
        ...
        elif action_order.get("reason") == "priority_advantage" and action_order.get("self_prankster_applied") is True and action_order.get("status") == "acts_first":
        ...
        elif action_order.get("reason") == "speed_advantage" and action_order.get("trick_room") == "active" and action_order.get("paralysis_speed_adjustment_applied") is True:
            order = "트릭룸에서는 마비로 느려진 쪽이 먼저 행동함"
        ...
        elif action_order.get("reason") == "speed_advantage" and action_order.get("self_paralysis") == "paralyzed" and action_order.get("status") == "acts_second":
    ...
    status_role = evidence.get("status_move_evidence")
    if isinstance(status_role, Mapping):
        if status_role.get("status") == "known_role" and isinstance(status_role.get("role_tags"), list):
            role_labels = {
                "recovery": "회복",
                "self_stat_raise": "자신 능력 상승",
                "target_stat_lower": "상대 능력 하락",
                "status_infliction": "상태이상 부여",
                "protection": "보호",
                "hazard_setup": "장판 설치",
                "hazard_removal": "장판 제거",
                "screen_setup": "보호막 설치",
                "field_or_weather_setup": "필드 또는 날씨 설정",
                "switching_or_phazing": "교체 또는 강제 교체",
                "utility_or_other": "기타 보조 효과",
            }
            shown_roles = [role_labels[tag] for tag in status_role["role_tags"] if tag in role_labels]
            if shown_roles:
                lines.append(f"기술 역할: {', '.join(shown_roles)}")
        elif status_role.get("status") == "insufficient_context":
