...
def _build_ui_selected_prompt(battle_input: dict[str, Any]) -> str:
    return (
        ...
        "treat it as raw Speed comparison only, not final turn order. If "
        ...
        "fallback is not used in v0.28; raw Speed comparison requires "
        "user-confirmed final Speed for both active Pokemon. Do not apply "
        "Choice Scarf speed, priority, Tailwind, Trick Room, paralysis, Speed "
        "stages, or ability speed effects unless explicit calculated fields "
        "say they are modeled. If item_profiles is present, "
        ...
        "Choice Band or Choice Specs is applied, say choice lock is not modeled. "
        "Choice lock, Life Orb recoil, Choice Scarf speed, Focus Sash survival, "
