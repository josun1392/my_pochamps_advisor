...
def _build_ui_selected_prompt(battle_input: dict[str, Any]) -> str:
    return (
        ...
        "and move accuracy metadata is available. Hit-adjusted KO probability "
        "is not calculated. Do not claim the move will miss or that a miss is "
        "guaranteed, and do not claim final hit probability unless explicitly "
        "calculated. Do not infer Bright Powder if the item is unknown or "
        "unconfirmed. Accuracy/evasion stages, ability interactions, weather, "
        "multi-hit accuracy, and turn sequencing are not modeled for "
        "accuracy_context. Focus Sash survival may appear only as limited "
