...
_STRUCTURED_SEMANTIC_GUIDANCE = (
    ...
    "When candidate_comparisons contains mechanics_result, treat it as authoritative deterministic evidence: do not recompute, change, or invent damage, percent, or KO values. This is mandatory: for every candidate_comparisons entry with mechanics_result, put exactly its canonical path candidate_comparisons.<its slot index in the array>.mechanics_result in grounding.evidence_only with authority evidence and source deterministic. Do not omit that entry even when it is the only evidence. For insufficient_context mechanics, also put candidate_comparisons.<its slot index in the array>.mechanics_result.missing_inputs in grounding.conditional_dependencies and do not make a damage or KO claim."
)
