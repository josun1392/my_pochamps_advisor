...
def _path_graph(*, strategy_d0: Mapping[str, Any], runtime_snapshot: Mapping[str, Any], base: Mapping[str, Any], target_hp: int, sturdy_survival_authority: Mapping[str, Any] | None, focus_sash_survival_authority: Mapping[str, Any] | None, contact_reactive_contact_authority: Mapping[str, Any] | None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], Fraction] | tuple[dict[str, str], None, None, None]:
    ...
    node_index: dict[tuple[int, int, int, int, bool, bool, int, str, bool], str] = {}
    ...
    def add_node(attempt: int, landed: int, maximum: int, hp: int, sturdy_consumed: bool, focus_sash_consumed: bool, attacker_hp: int, condition: str, path_local: bool) -> str:
        key = (attempt, landed, maximum, hp, sturdy_consumed, focus_sash_consumed, attacker_hp, condition, path_local)
        existing = node_index.get(key)
        if existing is not None:
            return existing
        node_id = f"attempt:{attempt}/landed:{landed}/max:{maximum}/hp:{hp}/attacker-hp:{attacker_hp}/condition:{condition}/sturdy:{'consumed' if sturdy_consumed else 'available'}/focus-sash:{'consumed' if focus_sash_consumed else 'available'}"
        node_index[key] = node_id
        nodes.append({"node_id": node_id, "attempt_index": attempt, "landed_hit_count": landed, "maximum_attempts": maximum, "target_hp": hp, "attacker_hp": attacker_hp, "attacker_condition": condition, "sturdy_consumed": sturdy_consumed, "focus_sash_consumed": focus_sash_consumed, "path_local": path_local})
        node_mass[node_id] = Fraction()
        return node_id

    for root_row, (maximum, probability) in zip(roots, planned_counts):
        root = add_node(1, 0, maximum, target_hp, False, False, base["own_current_hp"], base["attacker_condition"], False)
    ...
    while cursor < len(nodes):
        ...
        if events is None:
            current_d0, current_snapshot = (strategy_d0, runtime_snapshot) if not node["path_local"] else _detached_target_hp_view(
        ...
        for event in events:
            ...
            status_branches = _apply_reactive_status(
                ...
                hit_index=row["hit_index"], condition_state=node["attacker_condition"], attacker_fainted=attacker_hp == 0,
            )
            if isinstance(status_branches, Mapping):
                return {"status": status_branches.get("status", "rejected"), "reason": status_branches.get("reason", "population_bomb_contact_reactive_status_unavailable")}, None, None, None
            ...
            for status in status_branches:
                ...
                if terminal:
                    ...
                    next_node = add_node(attempt + 1, node["landed_hit_count"] + 1, node["maximum_attempts"], row["post_hp"], sturdy_consumed, focus_sash_consumed, attacker_hp, status["post_condition"], True)
...
def _base(d0: Any, action: Any, authority: Any) -> dict[str, Any] | None:
    ...
    if condition is None:
        return None
    return {"session_id": d0["session_id"], "source_runtime_fingerprint": d0["source_runtime_fingerprint"], "source_branch_fingerprint": d0["strategy_preview_fingerprint"], "decision_owner": deepcopy(dict(attacker)), "action_id": action["action_id"], "move_id": _MOVE_ID, "attacker": deepcopy(dict(attacker)), "target": deepcopy(dict(target)), "own_current_hp": _mapping(own_hp)["current_hp"], "attacker_condition": condition, "attacker_ability": _execution_attacker_ability(critical), "per_attempt_hit_probability": probabilities[0], "per_attempt_miss_probability": probabilities[1], "per_hit_critical_execution": deepcopy(dict(critical)), "single_hit_metadata_view": single, "execution_authority": deepcopy(dict(authority))}
